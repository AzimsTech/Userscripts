...
`.replaceAll(/const (\w+?) = require\('(.+?)'\);/g,(h,b,d)=>`
let ${b} = {};
(function (exports, module) { ${GM_getResourceText(d)} }) (
  ${b},
  {
    set exports(value) { ${b} = value },
    get exports() { return ${b} }
  }
);`),n=URL.createObjectURL(new Blob([r],{type:"text/javascript"}));setTimeout(URL.revokeObjectURL,0,n);const l=new Worker(n);unsafeWindow[tempName][t]=require("comlink").wrap(l);return}catch{supportWorker=!1}let a=`
  ...
  `;unsafeWindow[tempName]=crsLib,unsafeWindow[tempName][t]={},evalCode(a),Reflect.deleteProperty(unsafeWindow,tempName)},require=t=>{const o={value:!0},a=()=>{};a.default={};const r=new Proxy(a,{get(n,l){if(l==="__esModule")return o;if(l==="default")return r;crsLib[t]||selfImportSync(t);const h=crsLib[t];return Reflect.has(crsLib[t],"default")&&Reflect.has(crsLib[t].default,l)?h.default?.[l]:h?.[l]},apply(n,l,h){crsLib[t]||selfImportSync(t);const b=crsLib[t];return(typeof b.default=="function"?b.default:b)(...h)},construct(n,l){crsLib[t]||selfImportSync(t);const h=crsLib[t],b=typeof h.default=="function"?h.default:h;return new b(...l)}});return r};crsLib.require=require;const languages=require("helper/languages"),otherSite=require("userscript/otherSite"),helper=require("helper"),main=require("main");let options;try{switch(window.location.hostname){case"bbs.yamibo.com":{const t=require("solid-js/web"),o=require("solid-js"),a=require("main"),r=require("helper");(async()=>{const{options:n,setComicLoad:l,showComic:h,loadComic:b,setManga:d,needAutoShow:s}=await a.useInit("yamibo",{\u8BB0\u5F55\u9605\u8BFB\u8FDB\u5EA6:!0,\u5173\u95ED\u5FEB\u6377\u5BFC\u822A\u7684\u8DF3\u8F6C:!0,\u4FEE\u6B63\u70B9\u51FB\u9875\u6570\u65F6\u7684\u8DF3\u8F6C\u5224\u5B9A:!0,\u56FA\u5B9A\u5BFC\u822A\u6761:!0,\u81EA\u52A8\u7B7E\u5230:!0});if(GM_addStyle(`#fab { --fab: #6E2B19; }
    ...
    `),unsafeWindow.discuz_uid&&unsafeWindow.discuz_uid!=="0"&&n.\u81EA\u52A8\u7B7E\u5230&&(async()=>{const c=new Date().toLocaleDateString("zh-CN");if(c===localStorage.getItem("signDate"))return;const _=r.querySelector('#scbar_form > input[name="formhash"]')?.value;if(_)try{const p=await(await fetch(`plugin.php?id=zqlj_sign&sign=${_}`)).text();if(!/成功！|打过卡/.test(p))throw new Error("\u81EA\u52A8\u7B7E\u5230\u5931\u8D25");a.toast.success("\u81EA\u52A8\u7B7E\u5230\u6210\u529F"),localStorage.setItem("signDate",c)}catch{a.toast.error("\u81EA\u52A8\u7B7E\u5230\u5931\u8D25")}})(),n.\u5173\u95ED\u5FEB\u6377\u5BFC\u822A\u7684\u8DF3\u8F6C&&r.querySelector("#qmenu a")?.setAttribute("href","javascript:;"),/thread(-\d+){3}|mod=viewthread/.test(document.URL)){for(const m of r.querySelectorAll('img[file*="sinaimg.cn"]'))m.setAttribute("referrerpolicy","no-referrer");const c=()=>{!r.querySelector(".pg > .prev")||(s.val=!1);let p=r.querySelectorAll(":is(.t_fsz, .message) img");const k=()=>{let w=p.length;for(;w--;){const v=p[w],f=v.getAttribute("file");f&&v.src!==f&&(v.setAttribute("src",f),v.setAttribute("lazyloaded","true")),(v.src.includes("static/image")||v.complete&&v.naturalHeight&&v.naturalWidth&&v.naturalHeight<500&&v.naturalWidth<500)&&p.splice(w,1)}return p.map(v=>v.src)};l(k),d({onLoading(w,v){if(v&&v.width<500&&v.height<500)return b()},onExit(w){w&&r.scrollIntoView(".psth, .rate, #postlist > div:nth-of-type(2)"),d("show",!1)}}),r.querySelector("div.pti > div.authi")&&(r.querySelector("div.pti > div.authi").insertAdjacentHTML("beforeend",'<span class="pipe show">|</span><a id="comicReadMode" class="show" href="javascript:;">\u6F2B\u753B\u9605\u8BFB</a>'),document.getElementById("comicReadMode")?.addEventListener("click",()=>h())),r.querySelector("#threadindex")&&r.hijackFn("ajaxinnerhtml",()=>{p=r.querySelectorAll(".t_fsz img"),!(p.length===0||k().length===0)&&n.autoShow&&h()});const g=r.querySelector(".ptg.mbm.mtn > a");if(g){const w=g.href.split("id=")[1],v=/(?<=<th>\s<a href="thread-)\d+(?=-)/g;let f=[];const $=async(L=1)=>{const I=[...(await a.request(`/misc.php?mod=tag&id=${w}&type=thread&page=${L}`)).responseText.matchAll(v)].map(([u])=>Number(u));f=f.concat(I);const S=f.indexOf(unsafeWindow.tid);return I.length>0&&(S===-1||!f[S+1])?$(L+1):d({onPrev:f[S-1]?()=>window.location.assign(`thread-${f[S-1]}-1-1.html`):void 0,onNext:f[S+1]?()=>window.location.assign(`thread-${f[S+1]}-1-1.html`):void 0})};setTimeout($)}},_=unsafeWindow.fid??Number(new URLSearchParams(r.querySelector("h2 > a")?.href).get("fid")??"-1");if(_===30||_===37)c();else{r.querySelector("div.pti > div.authi").insertAdjacentHTML("beforeend",'<span class="pipe show">|</span><a id="comicReadMode" class="show" href="javascript:;">\u6F2B\u753B\u9605\u8BFB</a>');const m=document.getElementById("comicReadMode");m?.addEventListener("click",()=>{m.previousElementSibling?.remove(),m.remove(),c(),h()})}if(n.\u8BB0\u5F55\u9605\u8BFB\u8FDB\u5EA6){const m=unsafeWindow.tid??new URLSearchParams(window.location.search).get("tid")??/\/thread-(\d+)-\d+-\d+.html/.exec(window.location.pathname)?.[1];if(!m)return;let p;try{const M=await a.request(`/api/mobile/index.php?module=viewthread&tid=${m}`,{responseType:"json",errorText:"\u83B7\u53D6\u5E16\u5B50\u56DE\u590D\u6570\u65F6\u51FA\u9519",noTip:!0});p=Number.parseInt(M.response?.Variables?.thread?.allreplies,10)}catch{}const k=Number.parseInt(r.querySelector("#pgt strong")?.textContent??r.querySelector("#dumppage")?.value??"1",10),g=await r.useCache({history:"tid"}),w=await g.get("history",`${m}`);if(w&&k<w.lastPageNum)return;const v=r.querySelectorAll(w?.lastAnchor&&k===w.lastPageNum?`#${w.lastAnchor} ~ div`:"#postlist > div, .plc.cl");if(v.length===0)return;let f=0;const $=M=>{f&&window.clearTimeout(f),f=window.setTimeout(async()=>{f=0,await g.set("history",M)},200)},L=new IntersectionObserver(M=>{const I=M.find(u=>u.isIntersecting);if(!I)return;const S=v.indexOf(I.target);if(S!==-1){for(const u of v.splice(0,S+1))L.unobserve(u);$({tid:`${m}`,lastPageNum:k,lastReplies:p||w?.lastReplies||0,lastAnchor:I.target.id})}},{rootMargin:"-160px"});for(const M of v)L.observe(M)}return}if(/forum(-\d+){2}|mod=forumdisplay/.test(document.URL)){if(n.\u4FEE\u6B63\u70B9\u51FB\u9875\u6570\u65F6\u7684\u8DF3\u8F6C\u5224\u5B9A){const c=r.querySelectorAll(".tps>a");let _=c.length;for(;_--;)c[_].setAttribute("onClick","atarget(this)")}if(n.\u8BB0\u5F55\u9605\u8BFB\u8FDB\u5EA6){const c=await r.useCache({history:"tid"}),_=!document.querySelector("#flk"),[m,p]=o.createSignal(!1),k=()=>p(f=>!f);let g="tbody[id^=normalthread]",w=f=>f.id.split("_")[1],v=(f,$)=>`thread-${$}-${f.lastPageNum}-1.html#${f.lastAnchor}`;_&&(g=".threadlist li.list",w=f=>new URLSearchParams(f.children[1].getAttribute("href")).get("tid"),v=(f,$)=>`forum.php?mod=viewthread&tid=${$}&extra=page%3D1&mobile=2&page=${f.lastPageNum}#${f.lastAnchor}`);for(const f of r.querySelectorAll(g)){const $=w(f);t.render(()=>{const[L,M]=o.createSignal();r.createEffectOn(m,()=>c.get("history",$).then(M));const I=o.createMemo(()=>L()?v(L(),$):""),S=o.createMemo(()=>!_&&L()?Number(f.querySelector(".num a").innerHTML)-L().lastReplies:0),u=()=>[(()=>{var x=t.template("<a class=historyTag>\u56DE\u7B2C<!>\u9875 ")(),C=x.firstChild,P=C.nextSibling;return P.nextSibling,t.addEventListener(x,"click",unsafeWindow.atarget,!0),t.insert(x,()=>L()?.lastPageNum,P),t.effect(()=>t.setAttribute(x,"href",I())),x})(),t.createComponent(o.Show,{get when(){return S()>0},get children(){var x=t.template("<div class=historyTag>+")();return x.firstChild,t.insert(x,S,null),x}})],y=()=>(()=>{var x=t.template("<li><a>\u56DE\u7B2C<!>\u9875")(),C=x.firstChild,P=C.firstChild,T=P.nextSibling;return T.nextSibling,t.addEventListener(C,"click",unsafeWindow.atarget,!0),C.style.setProperty("color","unset"),t.insert(C,()=>L()?.lastPageNum,T),t.effect(()=>t.setAttribute(C,"href",I())),x})();return t.createComponent(o.Show,{get when(){return!!L()},get children(){return t.createComponent(o.Show,{when:_,get children(){return y()},get fallback(){return u()}})}})},_?f.children[3]:f.getElementsByTagName("th")[0])}document.addEventListener("visibilitychange",k),r.querySelector("#autopbn")?.addEventListener("click",k)}}})().catch(n=>r.log.error(n)),t.delegateEvents(["click"]);break}case"www.yamibo.com":{if(!window.location.pathname.includes("/manga/view-chapter"))break;const t=new URLSearchParams(window.location.search).get("id");if(!t)break;const o=Number(helper.querySelector("section div:first-of-type div:last-of-type").innerHTML.split("\uFF1A")[1]);if(Number.isNaN(o))throw new Error(helper.t("site.changed_load_failed"));const a=async n=>{const l=await main.request(`https://www.yamibo.com/manga/view-chapter?id=${t}&page=${n}`);return/(?<=<img id=['"]imgPic['"].+?src=['"]).+?(?=['"])/.exec(l.responseText)[0].replaceAll("&amp;","&").replaceAll("http://","https://")},r=n=>helper.plimit(helper.createSequence(o).map(l=>async()=>n(l,await a(l+1))));options={name:"newYamibo",getImgList:({dynamicLoad:n})=>n(r,o)(),onNext:helper.querySelectorClick("#btnNext"),onPrev:helper.querySelectorClick("#btnPrev"),onExit:n=>n&&helper.scrollIntoView("#w1")};break}case"comic.idmzj.com":case"comic.dmzj.com":case"manhua.idmzj.com":case"manhua.dmzj.com":{const t=require("solid-js/web"),o=require("solid-js"),a=require("main"),r=require("userscript/dmzjApi"),n=require("helper");(async()=>{const l=async()=>{const[,_,m]=window.location.pathname.split(/\/|\./);return _||a.toast.error("\u6F2B\u753B\u6570\u636E\u83B7\u53D6\u5931\u8D25",{duration:Number.POSITIVE_INFINITY,throw:new Error("\u83B7\u53D6\u6F2B\u753B\u62FC\u97F3\u7B80\u79F0\u5931\u8D25")}),{comicId:await r.getComicId(_),chapterId:m}},h=async()=>{if(await n.waitDom(".commentBox"),!n.querySelector(".cartoon_online_border > img"))return!1;n.querySelector(".cartoon_online_border").innerHTML="\u83B7\u53D6\u6F2B\u753B\u6570\u636E\u4E2D";for(const m of n.querySelectorAll(".odd_anim_title ~ *"))m.remove();const{comicId:_}=await l();return t.render(()=>{const m=r.useComicDetail(_);return t.createComponent(o.For,{get each(){return m.chapters},children:({name:p,list:k})=>[(()=>{var g=t.template('<div class=photo_part><div class=h2_title2><span class="h2_icon h2_icon22"></span><h2> ')(),w=g.firstChild,v=w.firstChild,f=v.nextSibling,$=f.firstChild;return t.insert(f,()=>m.title,$),t.insert(f,p==="\u8FDE\u8F7D"?"\u5728\u7EBF\u6F2B\u753B\u5168\u96C6":`\u6F2B\u753B\u5176\u5B83\u7248\u672C\uFF1A${p}`,null),g})(),(()=>{var g=t.template("<div class=cartoon_online_border_other><ul></ul><div class=clearfix>")(),w=g.firstChild;return g.style.setProperty("margin-top","-8px"),t.insert(w,t.createComponent(o.For,{each:k,children:({title:v,id:f,updatetime:$})=>(()=>{var L=t.template("<li><a target=_blank>")(),M=L.firstChild;return t.setAttribute(M,"title",v),t.setAttribute(M,"href",`https://m.dmzj.com/view/${_}/${f}.html`),t.insert(M,v),t.effect(()=>M.classList.toggle("color_red",$===m.last_updatetime)),L})()})),g})()]})},n.querySelector(".middleright_mr")),!1},b=async()=>{await n.waitDom("#qiehuan_txt"),await n.wait(()=>{const _=n.querySelector("#qiehuan_txt");if(_){if(_.textContent!=="\u5207\u6362\u5230\u4E0A\u4E0B\u6EDA\u52A8\u9605\u8BFB")return!0;_.click()}})},d=async()=>(await b(),await n.waitDom(".comic_wraCon img"),n.querySelectorAll(".comic_wraCon img").map(_=>_.src)),s=_=>{const m=n.querySelector(_);if(m?.textContent)return()=>m.click()},c=()=>/^\/[^/]*?\/?$/.test(window.location.pathname)?h():/^\/.*?\/\d+\.shtml$/.test(window.location.pathname);await a.universal({name:"dmzj",getImgList:d,onExit:_=>_&&n.scrollIntoView("#hd"),async getCommentList(){const{comicId:_,chapterId:m}=await l();return r.getViewpoint(_,m)},SPA:{isMangaPage:c,getOnPrev:()=>s(".display_left #prev_chapter"),getOnNext:()=>s(".display_right #next_chapter")}})})().catch(l=>n.log.error(l));break}case"m.idmzj.com":case"m.dmzj.com":{const t=require("dmzjDecrypt"),o=require("userscript/dmzjApi"),a=require("main"),r=require("helper");(async()=>{switch(window.location.pathname.split("/")[1]){case"info":{if(Reflect.has(unsafeWindow,"obj_id"))return;const n=Number.parseInt(window.location.pathname.split("/")[2],10);if(Number.isNaN(n)){document.body.innerHTML="",document.body.insertAdjacentHTML("beforeend",`
            \u8BF7\u624B\u52A8\u8F93\u5165\u6F2B\u753B\u540D\u8FDB\u884C\u641C\u7D22 <br />
            <input type="search"> <button>\u641C\u7D22</button> <br />
            <div id="list" />
          `),r.querySelector("button").addEventListener("click",async()=>{const s=r.querySelector("input")?.value;if(!s)return;const c=await a.request(`https://s.acg.dmzj.com/comicsum/search.php?s=${s}`,{errorText:"\u641C\u7D22\u6F2B\u753B\u65F6\u51FA\u9519"}),_=JSON.parse(c.responseText.slice(20,-1));r.querySelector("#list").innerHTML=_.map(({id:m,comic_name:p,comic_author:k,comic_url:g})=>`
                <b>\u300A${p}\u300B<b/>\u2014\u2014${k}
                <a href="${g}">Web\u7AEF</a>
                <a href="https://m.dmzj.com/info/${m}.html">\u79FB\u52A8\u7AEF</a>
              `).join("<br />")});return}const l=await a.request(`https://v4api.idmzj.com/comic/detail/${n}?uid=2665531&disable_level=1`,{errorText:"\u83B7\u53D6\u6F2B\u753B\u6570\u636E\u5931\u8D25"}),{comicInfo:{last_updatetime:h,title:b,chapters:d}}=t(l.responseText);document.title=b,document.body.insertAdjacentHTML("beforeend",`<h1>${b}</h1>`);for(const s of Object.values(d)){let c=`<h2>${s.title}</h2>`,_=s.data.length;for(;_--;)c+=`<a target="_blank" title="${s.data[_].chapter_title}" href="https://m.dmzj.com/view/${n}/${s.data[_].chapter_id}.html" ${s.data[_].updatetime===h?'style="color:red"':""}>${s.data[_].chapter_title}</a>`;document.body.insertAdjacentHTML("beforeend",c)}document.body.childNodes[0].remove(),GM_addStyle(`
          h1 {
            margin: 0 -20vw;
          }
        ...
        `);break}case"view":{if(unsafeWindow.comic_id){GM_addStyle(".subHeader{display:none !important}"),await a.universal({name:"dmzj",getImgList:()=>r.querySelectorAll("#commicBox img").map(w=>w.dataset.original).filter(Boolean),getCommentList:()=>o.getViewpoint(unsafeWindow.subId,unsafeWindow.chapterId),onNext:r.querySelectorClick("#loadNextChapter"),onPrev:r.querySelectorClick("#loadPrevChapter")});return}const n=document.createElement("p");n.textContent="\u6B63\u5728\u52A0\u8F7D\u4E2D\uFF0C\u8BF7\u5750\u548C\u653E\u5BBD\uFF0C\u82E5\u957F\u65F6\u95F4\u65E0\u53CD\u5E94\u8BF7\u5237\u65B0\u9875\u9762",document.body.append(n);let l,h,b;try{[,h,b]=/(\d+)\/(\d+)/.exec(window.location.pathname),l=await o.getChapterInfo(h,b)}catch(w){throw a.toast.error("\u83B7\u53D6\u6F2B\u753B\u6570\u636E\u5931\u8D25",{duration:Number.POSITIVE_INFINITY}),n.textContent=w.message,w}n.textContent="\u52A0\u8F7D\u5B8C\u6210\uFF0C\u5373\u5C06\u8FDB\u5165\u9605\u8BFB\u6A21\u5F0F";const{folder:d,chapter_name:s,next_chap_id:c,prev_chap_id:_,comic_id:m,page_url:p}=l;document.title=`${s} ${d.split("/").at(1)}`;const{setManga:k,setComicLoad:g}=await a.useInit("dmzj");k({onExit:void 0,onNext:c?()=>{window.location.href=`https://m.dmzj.com/view/${m}/${c}.html`}:void 0,onPrev:_?()=>{window.location.href=`https://m.dmzj.com/view/${m}/${_}.html`}:void 0,editButtonList:w=>w}),g(()=>p.length>0?p:(n.innerHTML='\u65E0\u6CD5\u83B7\u5F97\u6F2B\u753B\u6570\u636E\uFF0C\u8BF7\u901A\u8FC7 <a href="https://github.com/hymbz/ComicReadScript/issues" target="_blank">Github</a> \u6216 <a href="https://sleazyfork.org/zh-CN/scripts/374903-comicread/feedback#post-discussion" target="_blank">Greasy Fork</a> \u8FDB\u884C\u53CD\u9988',[])),k("commentList",await o.getViewpoint(h,b));break}}})().catch(n=>r.log.error(n));break}case"www.idmzj.com":case"www.dmzj.com":{const t=require("userscript/dmzjApi"),o=require("main"),a=require("helper"),r=n=>{if(n)return()=>{window.open(window.location.href.replace(/(?<=\/)\d+(?=\.html)/,`${n}`),"_self")}};(async()=>{await a.waitDom(".head_wz");const n=a.querySelector(".head_wz [id]")?.id,l=/(?<=\/)\d+(?=\.html)/.exec(window.location.pathname)?.[0];if(!n||!l)return;const{setManga:h,setComicLoad:b}=await o.useInit("dmzj");try{const{next_chap_id:d,prev_chap_id:s,page_url:c}=await t.getChapterInfo(n,l);b(()=>c),h({onNext:r(d),onPrev:r(s)})}catch{o.toast.error("\u83B7\u53D6\u6F2B\u753B\u6570\u636E\u5931\u8D25",{duration:Number.POSITIVE_INFINITY})}})().catch(n=>a.log.error(n));break}case"exhentai.org":case"e-hentai.org":{const web=require("solid-js/web"),solidJs=require("solid-js"),main=require("main"),Manga=require("components/Manga"),helper=require("helper"),store=require("solid-js/store"),ehTagRules=require("userscript/ehTagRules"),detectAd$1=require("userscript/detectAd"),createEhContext=async t=>{let o;if(Reflect.has(unsafeWindow,"display_comment_field")?o="gallery":location.pathname==="/mytags"?o="mytags":Reflect.has(unsafeWindow,"mpvkey")?o="mpv":o=helper.querySelector('option[value="t"]')?.parentElement?.value,!o)return null;const a=await main.useInit("ehentai",t);if(o!=="gallery")return{type:o,...a};let r=0;return r=Number(helper.querySelector(".gtb .gpc")?.textContent?.replaceAll(",","").match(/\d+/g)?.at(-1)),Number.isNaN(r)&&(r=Number(/(?<=<td class="gdt2">)\d+(?= pages<\/td>)/.exec((await main.request(window.location.href)).responseText)?.[0])),{type:"gallery",...a,galleryId:Number(location.pathname.split("/")[2]),galleryTitle:helper.querySelector("#gn")?.textContent||void 0,japanTitle:helper.querySelector("#gj")?.textContent||void 0,imgNum:r,imgList:[],pageList:[],fileNameList:[],LoadButton(n){const l=solidJs.createMemo(()=>{const h=a.comicMap[n.id]?.imgList,b=h?.filter(Boolean).length;switch(h?.length){case void 0:return" Load comic";case b:return" Read";default:return` loading - ${b}/${h.length}`}});return(()=>{var h=web.template("<a href=javascript:;>")();return h.$$click=async b=>{await n.onClick?.(b),a.showComic(n.id)},web.insert(h,l),h})()},dom:{newTagField:document.getElementById("newtagfield")}}};web.delegateEvents(["click"]);const escHandler=[],setEscHandler=(t,o)=>{escHandler.push(Object.assign(o,{order:t})),escHandler.sort((a,r)=>r.order-a.order)},getTaglist=()=>{const t=new Set,o=new Set;for(const a of helper.querySelectorAll("#taglist table [id^=td_]")){const[r]=a.getElementsByTagName("a");r.classList.contains("tdn")||(r.classList.contains("tup")||a.classList.contains("gt")?t.add(a.id.slice(3)):a.classList.contains("gtl")&&o.add(a.id.slice(3)))}return[t,o]},handleTagName=t=>{const[o,a]=t.trim().split(":");return a?[o,a.replaceAll(/[^a-zA-Z-_ ]/g,"")]:["",""]},namespaceAbbr=[["artist","a"],["character","c","char"],["cosplayer","c","os"],["female","f"],["group","g","circle"],["language","l","lang"],["male","m"],["mixed","x"],["other","o"],["parody","p","series"],["reclass","r"]],getTagNameFull=t=>{const[o,a]=handleTagName(t);for(const r of namespaceAbbr)if(r.includes(o))return`${r[0]}:${a}`;return t};let hasStyle=!1;const addQuickFavorite=(favoriteButton,root,apiUrl,position)=>{hasStyle||(hasStyle=!0,GM_addStyle(`
      .comidread-favorites {
        position: absolute;
        z-index: 75;
        left: 0;
...
`,o+=await helper.getGmValue("ehTagColorizeCss",updateMyTags),GM_addStyle(o)}case"mytags":updateMyTags(),helper.hijackFn("usertag_callback",helper.debounce(updateMyTags))}},quickRating=t=>{let o;switch(t.type){case"e":o=helper.querySelectorAll("#favform > table > tbody > tr");break;case"m":case"p":case"l":o=helper.querySelectorAll("#favform > table > tbody > tr").slice(1);break;case"t":o=helper.querySelectorAll(".gl1t");break;default:return}GM_addStyle(`
  ...
  `);const a=["0,0,7,16","8,0,15,16","16,0,23,16","24,0,31,16","32,0,39,16","40,0,47,16","48,0,55,16","56,0,63,16","64,0,71,16","72,0,79,16"],r=async(h,b)=>{try{const d=await main.request(h,{errorText:helper.t("site.ehentai.change_rating_failed"),noTip:!0}),s=/api_url = "(.+?)".+?gid = (\d+).+?token = "(.+?)".+?apiuid = (\d+).+?apikey = "(.+?)"/s.exec(d.responseText);if(!s)throw new Error(helper.t("site.ehentai.change_rating_failed"));const[,c,_,m,p,k]=s,g=await main.request(c,{method:"POST",responseType:"json",data:JSON.stringify({method:"rategallery",rating:`${b}`,apikey:k,apiuid:p,gid:_,token:m}),fetch:!0,noTip:!0});return main.toast.success(`${helper.t("site.ehentai.change_rating_success")}: ${g.response.rating_usr}`),g.response}catch{throw main.toast.error(helper.t("site.ehentai.change_rating_failed")),new Error(helper.t("site.ehentai.change_rating_failed"))}},n=(h,b)=>{let d=Math.round(b+1);const s=-80+16*Math.ceil(d/2);d=d%2===1?-21:-1,h.style.backgroundPosition=`${s}px ${d}px`},l=(h,b,d)=>{let s=b.style.backgroundPosition;web.render(()=>(()=>{var c=web.template("<span class=comidread-quick-rating><img src=https://ehgt.org/g/blank.gif><map>")(),_=c.firstChild,m=_.nextSibling;return c.$$mouseout=()=>{b.style.backgroundPosition=s},web.setAttribute(c,"data-index",d),web.setAttribute(_,"usemap",`#rating-${d}`),web.setAttribute(m,"name",`rating-${d}`),web.insert(m,web.createComponent(solidJs.For,{each:a,children:(p,k)=>(()=>{var g=web.template("<area shape=rect>")();return g.$$click=async()=>{const w=await r(h.querySelector("a").href,k()+1);b.className=w.rating_cls,n(b,w.rating_usr*2-1),s=b.style.backgroundPosition},g.$$mouseover=()=>n(b,k()),web.setAttribute(g,"coords",p),g})()})),c})(),b)};for(const[h,b]of o.entries()){const d=[...b.querySelectorAll(".ir")].at(-1);d&&d.addEventListener("mouseenter",()=>l(b,d,h),{once:!0})}};web.delegateEvents(["mouseout","mouseover","click"]);const MDLaunch=(t={})=>(()=>{var o=web.template('<svg xmlns=http://www.w3.org/2000/svg viewBox="0 0 24 24"stroke=currentColor fill=currentColor stroke-width=0><path d="M18 19H6c-.55 0-1-.45-1-1V6c0-.55.45-1 1-1h5c.55 0 1-.45 1-1s-.45-1-1-1H5a2 2 0 0 0-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2v-6c0-.55-.45-1-1-1s-1 .45-1 1v5c0 .55-.45 1-1 1M14 4c0 .55.45 1 1 1h2.59l-9.13 9.13a.996.996 0 1 0 1.41 1.41L19 6.41V9c0 .55.45 1 1 1s1-.45 1-1V3h-6c-.55 0-1 .45-1 1">')();return web.spread(o,t,!0,!0),o})(),quickTagDefine=t=>{const o=store.createMutable({}),a=async d=>{if(Reflect.has(o,d))return;const s=`https://ehwiki.org/wiki/${d.replaceAll(/[a-z]+:\s?/gi,"")}`,c=await main.request(s,{noCheckCode:!0});if(c.status!==200){o[d]=(()=>{var p=web.template("<h3>")();return web.insert(p,()=>`${c.status} - ${c.statusText}`),p})();return}const m=helper.domParse(c.responseText).querySelector("#mw-content-text");for(const p of m.querySelectorAll('img[src^="/"]'))p.setAttribute("src",`https://ehwiki.org${p.getAttribute("src")}`);for(const p of m.getElementsByTagName("a")){const k=p.getAttribute("href")??"";k.startsWith("/")&&p.setAttribute("href",`https://ehwiki.org${k}`),p.target="_blank"}for(const p of m.querySelectorAll(".thumb"))p.remove();o[d]=[(()=>{var p=web.template("<h1><a target=_blank>")(),k=p.firstChild;return web.setAttribute(k,"href",s),web.insert(k,d,null),web.insert(k,web.createComponent(MDLaunch,{}),null),p})(),m]};GM_addStyle(`
  ...
  `);const[r,n]=solidJs.createSignal(!1),l=helper.querySelector("#taglist");let h="rgba(0, 0, 0, 0)",b=l;for(;h==="rgba(0, 0, 0, 0)";)h=getComputedStyle(b).backgroundColor,b=b.parentElement;web.render(()=>web.createComponent(solidJs.Show,{get when(){return r()},get children(){var d=web.template("<span id=comidread-tag-define>")();return h!=null?d.style.setProperty("background",h):d.style.removeProperty("background"),web.insert(d,()=>o[unsafeWindow.selected_tagname]??web.template("<h3>loading...")()),web.effect(s=>(s=`${l.scrollHeight}px`)!=null?d.style.setProperty("height",s):d.style.removeProperty("height")),d}}),l),unsafeWindow.tag_define=async()=>{if(unsafeWindow.selected_tagname){if(r())return n(!1);n(!0);try{await a(unsafeWindow.selected_tagname)}catch(d){console.error(d),n(!1)}}},helper.hijackFn("toggle_tagmenu",()=>n(!1)),setEscHandler(2,()=>r()?n(!1):!0)},MdPictureInPicture='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke="currentColor" fill="currentColor" stroke-width="0"><path d="M18 7h-6c-.55 0-1 .45-1 1v4c0 .55.45 1 1 1h6c.55 0 1-.45 1-1V8c0-.55-.45-1-1-1m3-4H3c-1.1 0-2 .9-2 2v14c0 1.1.9 1.98 2 1.98h18c1.1 0 2-.88 2-1.98V5c0-1.1-.9-2-2-2m-1 16.01H4c-.55 0-1-.45-1-1V5.98c0-.55.45-1 1-1h16c.55 0 1 .45 1 1v12.03c0 .55-.45 1-1 1"/></svg>',getDomPosition=t=>{const o=t.getBoundingClientRect(),a=getComputedStyle(t),r=Number.parseFloat(a.borderLeftWidth),n=Number.parseFloat(a.paddingLeft),l=Number.parseFloat(a.paddingTop),h=Number.parseFloat(a.borderTopWidth);return{left:o.left+r+n,top:o.top+h+l,width:a.width,height:a.height}},floatTagList=({mangaProps:t,dom:{newTagField:o}})=>{const a=helper.querySelector("#gd4"),r=getComputedStyle(a);let n="rgba(0, 0, 0, 0)",l=a;for(;n==="rgba(0, 0, 0, 0)";)n=getComputedStyle(l).backgroundColor,l=l.parentElement;const{borderColor:h}=getComputedStyle(helper.querySelector("#gdt")),b=`1px solid ${h}`;GM_addStyle(`
      #comicread-tag-box {
        position: fixed;
        z-index: 2147483647;
    ...
    `);const{store:d,setState:s,_setState:c,_state:_}=helper.useStore({open:!1,top:0,left:0,opacity:1,mouse:{x:0,y:0},bound:{width:0,height:0}}),m=(u,y,x)=>{u.top=helper.clamp(-a.clientHeight*.75,y,u.bound.height),u.left=helper.clamp(-a.clientWidth*.75,x,u.bound.width)},p=u=>c("opacity",helper.clamp(.5,u,1));p(Number(localStorage.getItem("floatTagListOpacity"))||1),document.addEventListener("pointermove",u=>{s(y=>{y.mouse.x=u.clientX,y.mouse.y=u.clientY})});const k=()=>{s(u=>{u.bound.width=window.innerWidth-a.clientWidth/4,u.bound.height=window.innerHeight-a.clientHeight/4,m(u,u.top,u.left)})};window.addEventListener("resize",k),k(),helper.useStyleMemo("#comicread-tag-box",{display:()=>d.open?void 0:"none",top:()=>`${d.top}px`,left:()=>`${d.left}px`,opacity:()=>d.opacity});const g=a.cloneNode();g.id="comicread-tag-box-placeholder",g.style.display="none",g.addEventListener("click",()=>c("open",!1)),g.innerHTML=MdPictureInPicture,a.before(g);const w=document.createElement("div");w.id="comicread-tag-box",w.classList.add("comicread-ignore"),document.body.append(w),w.addEventListener("wheel",u=>{u.shiftKey&&(u.stopPropagation(),u.preventDefault(),p(d.opacity+(u.deltaY>0?-.05:.05)),localStorage.setItem("floatTagListOpacity",`${d.opacity}`))},{passive:!1});const v={top:0,left:0};helper.useDrag({ref:a,handleDrag({type:u,xy:[y,x],initial:[C,P]}){switch(u){case"down":if(!d.open){const T=getDomPosition(a);s(A=>{A.top=T.top,A.left=T.left})}v.top=d.top,v.left=d.left;break;case"up":s(T=>{if(t.show)return;const A=g.getBoundingClientRect();helper.approx(T.top,A.top,50)&&helper.approx(T.left,A.left,50)&&(T.open=!1)});break;case"move":s(T=>{m(T,v.top+x-P,v.left+y-C),T.open=!0});break}},handleClick:(u,y)=>y.click(),skip:u=>!u.target.matches("#gd4, #taglist, #gwrd, td+td, [id^=comidread] *:not(a)")});let f,$;const L=()=>{if(f||(f=helper.querySelector("#ehs-introduce-box"),!f))return;$=f.parentElement;const u=helper.querySelector(".eh-syringe-lite-auto-complete-list");u&&(u.classList.add("comicread-ignore"),u.style.zIndex="2147483647",document.body.append(u)),helper.hijackFn("toggle_tagmenu",()=>unsafeWindow.selected_tagname||helper.querySelector("#ehs-introduce-box .ehs-close")?.click())};helper.createEffectOn(()=>d.open,u=>{if(L(),u){const{height:y,width:x}=r;g.style.cssText=`height: ${y}; width: ${x};`,w.style.height=y,a.style.width=x,w.append(a),f&&w.append(f),document.activeElement.blur()}else g.style.cssText="display: none;",a.style.width="",g.after(a),f&&$.append(f),Manga.focus()},{defer:!0}),Manga.setDefaultHotkeys(u=>({...u,float_tag_list:["q"]})),setEscHandler(0,()=>d.open?c("open",!1):!0),helper.linstenKeydown(u=>{const y=helper.getKeyboardCode(u);Manga.hotkeysMap()[y]==="float_tag_list"&&(u.stopPropagation(),u.preventDefault(),s(x=>{x.open=!x.open,x.open&&m(x,x.mouse.y-a.clientHeight/2,x.mouse.x-a.clientWidth/2)}))}),helper.hijackFn("tag_from_field",(u,y)=>(d.open&&document.activeElement.blur(),u(...y))),o.addEventListener("pointerenter",()=>d.open&&o.focus());const M=u=>{const y=helper.querySelector(`a[href=${CSS.escape(u)}]`);if(y)return y.title||y.id.slice(3).replaceAll("_"," ")},I=u=>{const y=u.dataTransfer.getData("text"),x=M(y);x&&(u.preventDefault(),o.value.includes(x)||(o.value+=`${x}, `),o.dispatchEvent(new Event("input")))};o.addEventListener("drop",I);const S=helper.querySelector("#taglist");S.addEventListener("dragover",u=>u.preventDefault()),S.addEventListener("dragenter",u=>u.preventDefault()),S.addEventListener("drop",I)},updateSortCss=t=>{let o="tr a :is(.gltm, .glink + div:not([class])) { display: flex; }";for(const{title:a,order:r}of t)o+=`
.gt[title="${a}"] { order: ${r}; }`;return GM.setValue("ehTagSortCss",o)},sortTags=async t=>{switch(handleMyTagsChange.add(updateSortCss),t.type){case"p":case"l":case"t":return GM_addStyle(await helper.getGmValue("ehTagSortCss",updateMyTags));case"mytags":{let o;const a=r=>{let n=`
          ...
          #usertag_0 { order: -${r.length}; }`;for(const{order:l,id:h}of r)n+=`
#usertag_${h} { view-transition-name: _${h}; order: ${l}; }`;o||=GM_addElement("style",{textContent:n}),o.textContent=n};handleMyTagsChange.add(r=>{if(!document.startViewTransition)return a(r);document.startViewTransition(()=>a(r))})}}},tagLint=({dom:{newTagField:t}})=>{const o=helper.querySelector("#gdc > .cs:is(.ct2, .ct3, .ct9)"),a=ehTagRules.getTagLintRules(),[r,n]=solidJs.createSignal({});GM_addStyle(`
    #comidread-tag-lint [id^=td_] {
      display: inline-block;
      float: none;
    }
  `);const l=(g,w)=>w===void 0?document.getElementById(`td_${g}`)?.className:w?"gtl":"gt",h=g=>(()=>{var w=web.template("<div><a>")(),v=w.firstChild;return v.$$click=f=>f.preventDefault(),web.insert(v,()=>g.name),web.effect(f=>{var $=`td_${g.name}`,L=l(g.name,g.weak),M=`ta_${g.name}`,I=`https://exhentai.org/tag/${g.name.replaceAll("_","+")}`;return $!==f.e&&web.setAttribute(w,"id",f.e=$),L!==f.t&&web.className(w,f.t=L),M!==f.a&&web.setAttribute(v,"id",f.a=M),I!==f.o&&web.setAttribute(v,"href",f.o=I),f},{e:void 0,t:void 0,a:void 0,o:void 0}),w})(),b=g=>{const w=ehTagRules.splitTagNamespace(g.name);return web.createComponent(solidJs.Show,{get when(){return w.length>1},get fallback(){return h(g)},get children(){var v=web.template("<span>\u300C<!> \u300D")(),f=v.firstChild,$=f.nextSibling;return $.nextSibling,web.insert(v,web.createComponent(solidJs.For,{each:w,children:(L,M)=>[web.memo(()=>web.memo(()=>!!M())()?` ${helper.t("other.or")} `:""),web.createComponent(h,{name:L,get weak(){return g.weak}})]}),$),v}})},d=g=>{const[w,v,f]=g.text.split("[tag]");return web.createComponent(solidJs.Show,{get when(){return g.warnList?.size},get children(){return web.createComponent(solidJs.For,{get each(){return[...g.warnList.entries()]},children:([$,L])=>(()=>{var M=web.template("<li>")();return web.insert(M,w,null),web.insert(M,web.createComponent(b,{name:$}),null),web.insert(M,v,null),web.insert(M,web.createComponent(solidJs.For,{each:L,children:I=>web.createComponent(b,{name:I,get weak(){return g.weak}})}),null),web.insert(M,f,null),M})()})}})};let s,c;const _=helper.singleThreaded(()=>{const g={},[w,v]=getTaglist(),f=new Set([...w,...v]),$=(I,S,u=!1)=>{const y=a[S];if(y.has(I))for(const x of y.get(I)){if(ehTagRules.hasTag(u?w:f,x)===u)continue;g[S]||=new Map([[I,[]]]);const C=g[S];C.has(I)||C.set(I,[]),C.get(I).push(x)}};for(const I of f)$(I,"prerequisite",!0),$(I,"conflict"),o&&$(I,"possibleConflict"),$(I,"combo",!0);const L=(I,S)=>{g.other||=[],g.other.push([I,S])},M=[];for(const I of v)if(/^(artist|group):/.test(I)){const S=helper.querySelector("#gd2").textContent.toLowerCase();if(S.includes(I.replaceAll(/^(artist|group):|_/g," ").trim()))M.push(I);else{const u=document.getElementById(`ta_${I}`)?.textContent;u&&S.includes(u)&&M.push(I)}}M.length>0&&L(helper.t("eh_tag_lint.correct_tag"),M),helper.querySelector("#gdc > .cs.ct2")&&ehTagRules.isMissingNamespace(f,"parody")&&L(helper.t("eh_tag_lint.miss_parody"),["parody:original"]),o&&ehTagRules.isMissingTags(w,"female:females_only","female:futanari","female:shemale")&&ehTagRules.isMissingNamespace(f,"male","mixed")&&L(helper.t("eh_tag_lint.miss_female"),["female:females_only"]),n(g),s?.isConnected||(s=document.createElement("div"),s.id="comidread-tag-lint",helper.querySelector("#taglist").append(s)),c?.(),c=web.render(()=>web.createComponent(solidJs.Show,{get when(){return Object.keys(r()).length},get children(){return[web.template("<hr>")(),(()=>{var I=web.template("<ul>")();return web.insert(I,web.createComponent(solidJs.For,{get each(){return r().other},children:([S,u])=>(()=>{var y=web.template("<li>")();return web.insert(y,S,null),web.insert(y,web.createComponent(solidJs.For,{each:u,children:x=>web.createComponent(b,{name:x,weak:!0})}),null),y})()}),null),web.insert(I,web.createComponent(d,{get warnList(){return r().prerequisite},get text(){return helper.t("eh_tag_lint.prerequisite")},weak:!1}),null),web.insert(I,web.createComponent(d,{get warnList(){return r().conflict},get text(){return helper.t("eh_tag_lint.conflict")}}),null),web.insert(I,web.createComponent(d,{get warnList(){return r().possibleConflict},get text(){return helper.t("eh_tag_lint.possible_conflict")}}),null),web.insert(I,web.createComponent(d,{get warnList(){return r().combo},get text(){return helper.t("eh_tag_lint.combo")},weak:!0}),null),I})()]}}),s)});_(),helper.hijackFn("tag_update_vote",_);const[m,p]=helper.createEqualsSignal([]);helper.useStyle(helper.createRootMemo(()=>m().map(g=>`#td_${CSS.escape(g.replaceAll(" ","_"))} { box-shadow: 0px 0px 4px var(--tag); }`).join(`
`)));const k=()=>p(t.value.split(",").map(g=>getTagNameFull(g.trim())).filter(Boolean));t.addEventListener("input",k),t.addEventListener("keydown",k),helper.hijackFn("tag_update_vote",k)};web.delegateEvents(["click"]);const detectAd=async({setComicMap:t,options:o,comicMap:a,imgList:r,pageList:n,fileNameList:l})=>{if(!(o.detect_ad&&document.getElementById("ta_other:extraneous_ads")))return;t("",{adList:new main.ReactiveSet});const b=[];for(const d of helper.querySelectorAll("#gdt > a")){const s=Number(/.+-(\d+)/.exec(d.href)?.[1])-1;if(Number.isNaN(s))continue;n[s]=d.href;const c=d.querySelector("[title]");l[s]=c.title.split(/：|: /)[1],b[s]=c.tagName==="IMG"?c:/url\("(.+)"\)/.exec(c.style.backgroundImage)[1]}return(async()=>(await detectAd$1.getAdPageByFileName(l,a[""].adList),a[""].adList.size===0&&await detectAd$1.getAdPageByContent(b,a[""].adList),helper.useStyle(helper.createRootMemo(()=>a[""]?.adList?.size?[...a[""].adList].map(d=>`a[href="${n[d]}"] [title]:not(:hover) {
              filter: blur(8px);
              clip-path: border-box;
              backdrop-filter: blur(8px);
            }`).join(`
`):""))))(),()=>{detectAd$1.getAdPageByFileName(l,a[""].adList),detectAd$1.getAdPageByContent(r,a[""].adList)}};(async()=>{const t=await createEhContext({cross_site_link:!0,hotkeys:!0,detect_ad:!0,quick_favorite:!0,colorize_tag:!1,quick_rating:!0,quick_tag_define:!0,float_tag_list:!1,auto_adjust_option:!1,tag_lint:!1,autoShow:!1});if(!t)return;const{options:o,setComicLoad:a,dynamicLoad:r,showComic:n,setComicMap:l,setImgList:h,setFab:b,setManga:d}=t;if(t.type==="mpv")return a(()=>{const S=helper.querySelectorAll(".mimg[id]");return r(async y=>{const x=unsafeWindow.imagelist;helper.plimit(x.map((C,P)=>async()=>{const T=()=>x[P].i;for(;!T();)Reflect.has(x[P],"xhr")||(unsafeWindow.load_image(P+1),unsafeWindow.next_possible_request=0),await helper.wait(T);y(P,T())}),void 0,4)},S.length)()});if(helper.linstenKeydown(S=>{if(S.key==="Escape"){for(const u of escHandler)if(u()!==!0)return S.stopImmediatePropagation()}}),o.colorize_tag&&(colorizeTag(t),sortTags(t)),unsafeWindow.apiuid!==-1&&o.quick_favorite&&helper.requestIdleCallback(()=>quickFavorite(t)),o.quick_rating&&helper.requestIdleCallback(()=>quickRating(t),1e3),t.type!=="gallery")return hotkeysPageTurn(t);if(o.auto_adjust_option&&!helper.querySelector("#gdc > .cs:is(.ct2, .ct3, .ct9)")){let S={pageNum:1,imgRecognition:{enabled:!1}};o.option&&(S=helper.assign(o.option,S)),d({option:S})}o.float_tag_list&&helper.requestIdleCallback(()=>floatTagList(t)),o.quick_tag_define&&helper.requestIdleCallback(()=>quickTagDefine(),1e3),o.tag_lint&&helper.requestIdleCallback(()=>tagLint(t),1e3);const s=document.getElementById("gd5");if(new ResizeObserver(()=>{Reflect.deleteProperty(s.dataset,"long"),s.scrollHeight>352&&(s.dataset.long="")}).observe(s),GM_addStyle(`
    #gd5[data-long] {
  ...
  `),o.cross_site_link&&helper.requestIdleCallback(()=>crossSiteLink(t),1e3),Number.isNaN(t.imgNum))return main.toast.error(helper.t("site.changed_load_failed"));const _=await detectAd(t),m=S=>S.includes("IP address has been temporarily banned")&&main.toast.error(helper.t("site.ehentai.ip_banned"),{throw:!0,duration:Number.POSITIVE_INFINITY}),p=async S=>{const u=await main.request(S,{fetch:!0,errorText:helper.t("site.ehentai.fetch_img_page_source_failed")},10);m(u.responseText);try{return/id="img" src="(.+?)"/.exec(u.responseText)[1]}catch{throw new Error(helper.t("site.ehentai.fetch_img_url_failed"))}},k=async(S=0)=>{const u=await main.request(`${window.location.pathname}${S?`?p=${S}`:""}`,{fetch:!0,errorText:helper.t("site.ehentai.fetch_img_page_url_failed")});m(u.responseText);const y=[...u.responseText.matchAll(/<a href="(.{20,50})"><(img alt=.+?|div><div |div )title=".+?: (.+?)"/gm)].map(([,x,,C])=>[x,C]);if(y.length===0)throw new Error(helper.t("site.ehentai.fetch_img_page_url_failed"));return y},[g,w]=solidJs.createSignal(`1-${t.imgNum}`),v=helper.createRootMemo(()=>helper.extractRange(g(),t.imgList.length||t.imgNum)),f=Number(helper.querySelector(".ptt td:nth-last-child(2)").textContent);a(r(async S=>{if(t.pageList.length!==f){const u=await helper.plimit(helper.createSequence(f).map(y=>()=>k(y)));t.pageList.length=0,t.fileNameList.length=0;for(const y of u)for(const[x,C]of y)t.pageList.push(x),t.fileNameList.push(C)}await helper.plimit([...v()].map((u,y)=>async()=>{u<0||(t.imgList[u]||=await p(t.pageList[u]),S(y,t.imgList[u]))})),_?.()},()=>v().size));const L=await helper.useCache({pageRange:"id"});web.render(()=>{const S=s.children[6]?.classList.contains("gsp"),u=async y=>{if(!y.shiftKey)return;y.stopPropagation();const x=prompt(helper.t("other.page_range"),(await L.get("pageRange",unsafeWindow.gid))?.range);x&&(await L.set("pageRange",{id:unsafeWindow.gid??t.galleryId,range:x}),w(x??`1-${t.imgNum}`),l("","imgList",void 0),n())};return(()=>{var y=web.template('<p class="g2 gsp"><img src=https://ehgt.org/g/mr.gif>')();return y.firstChild,y.style.setProperty("padding-bottom","0"),(S?0:void 0)!=null?y.style.setProperty("padding-top",S?0:void 0):y.style.removeProperty("padding-top"),y.addEventListener("click",u,!0),web.insert(y,web.createComponent(t.LoadButton,{id:""}),null),y})()},s),o.hotkeys&&hotkeysPageTurn(t);const M=async S=>{const u=await main.request(S,{errorText:helper.t("site.ehentai.fetch_img_page_source_failed")});m(u.responseText);const y=/nl\('(.+?)'\)/.exec(u.responseText)?.[1];if(!y)throw new Error(helper.t("site.ehentai.fetch_img_url_failed"));const x=new URL(S);return x.searchParams.set("nl",y),x.href},I=helper.singleThreaded(async(S,u)=>{const y=t.imgList.indexOf(u);if(y!==-1){if(t.imgList[y]=await p(t.pageList[y]),!await helper.testImgUrl(t.imgList[y])&&(t.pageList[y]=await M(t.pageList[y]),t.imgList[y]=await p(t.pageList[y]),main.toast.warn(helper.t("alert.retry_get_img_url",{i:y})),!await helper.testImgUrl(t.imgList[y])))return await helper.sleep(500),I(u);h("",y,t.imgList[y]);for(const x of Manga.imgList())if(x.loadType==="error")return I(x.src)}});d({title:t.japanTitle||t.galleryTitle,onExit(S){S&&helper.scrollIntoView("#cdiv"),d("show",!1)},onImgError:I}),b("initialShow",o.autoShow)})().catch(t=>helper.log.error(t));break}case"nhentai.net":{const t=require("solid-js/web"),o=require("main"),a=require("userscript/detectAd"),r=require("helper"),n={j:"jpg",p:"png",g:"gif",w:"webp",b:"bmp"};(async()=>{const{options:l,setFab:h,setManga:b,setComicLoad:d,showComic:s,comicMap:c,setComicMap:_}=await o.useInit("nhentai",{auto_page_turn:!0,block_totally:!0,open_link_new_page:!0,detect_ad:!0});if(Reflect.has(unsafeWindow,"gallery")){b({onExit(g){g&&r.scrollIntoView("#comment-container"),b("show",!1)}});const m=unsafeWindow._n_app.options.media_server;d(()=>_gallery.images.pages.map((g,w)=>`https://i${m}.nhentai.net/galleries/${_gallery.media_id}/${w+1}.${n[g.t]}`)),h("initialShow",l.autoShow);const p=(()=>{var g=t.template('<a href=javascript:; id=comicReadMode class="btn btn-secondary"><i class="fa fa-book"></i> Read')();return g.$$click=()=>s(),g})();document.getElementById("download").after(p),l.detect_ad&&r.querySelector("#tags .tag.tag-144644")&&(_("","adList",new o.ReactiveSet),await a.getAdPageByContent(r.querySelectorAll(".thumb-container img").map(g=>g.dataset.src),c[""].adList),r.createEffectOn(()=>c[""].imgList,g=>g?.length&&a.getAdPageByContent(g,c[""].adList)),r.useStyle(()=>c[""]?.adList?.size?[...c[""].adList].map(w=>`
            .thumb-container:nth-of-type(${w+1}):not(:hover) {
              filter: blur(8px);
              clip-path: border-box;
            }`).join(`
`):""));return}if(document.getElementsByClassName("gallery").length>0){if(l.open_link_new_page)for(const p of r.querySelectorAll('a:not([href^="javascript:"])'))p.setAttribute("target","_blank");const m=(unsafeWindow?._n_app??unsafeWindow?.n)?.options?.blacklisted_tags;if(m===void 0&&o.toast.error(r.t("site.nhentai.tag_blacklist_fetch_failed")),l.block_totally&&m?.length&&GM_addStyle(".blacklisted.gallery { display: none; }"),l.auto_page_turn){let p=r.querySelector("a.next")?.href;if(!p)return;GM_addStyle(`
      ...
      `);const k=new Set(m),g=document.getElementById("content"),w=()=>g.querySelector(":is(.index-container, #favcontainer):last-of-type"),v=r.singleThreaded(async()=>{if(!p)return;const $=await o.request(p,{fetch:!0,errorText:r.t("site.nhentai.fetch_next_page_failed")}),L=r.domParse($.responseText);history.replaceState(null,"",p);const M=L.querySelector(".index-container, #favcontainer");for(const u of M.querySelectorAll(".gallery")){for(const x of u.getElementsByTagName("img"))x.setAttribute("src",x.dataset.src);u.dataset.tags.split(" ").map(Number).some(x=>k.has(x))&&u.classList.add("blacklisted")}const I=L.querySelector(".pagination");p=I.querySelector("a.next")?.href,g.append(M,I);const S=document.createElement("hr");g.append(S),f.disconnect(),f.observe(w()),p||(S.style.animationPlayState="paused")},{abandon:!0});v();const f=new IntersectionObserver($=>$[0].isIntersecting&&v());f.observe(w()),r.querySelector("section.pagination")&&g.append(document.createElement("hr"))}}})().catch(l=>r.log.error(l)),t.delegateEvents(["click"]);break}case"yuri.website":{const t=require("main"),o=require("helper");(async()=>{const{options:a,setManga:r,setComicLoad:n,showComic:l,comicMap:h,needAutoShow:b}=await t.useInit("yurifans",{\u81EA\u52A8\u7B7E\u5230:!0});if(a.\u81EA\u52A8\u7B7E\u5230&&(async()=>{if(!globalThis.b2token)return;const d=new Date().toLocaleDateString("zh-CN");if(d!==localStorage.getItem("signDate"))try{const s=await t.request("/wp-json/b2/v1/userMission",{method:"POST",noTip:!0,headers:{Authorization:`Bearer ${b2token}`}}),c=JSON.parse(s.responseText);if(!(c?.mission?.date||!Number.isNaN(Number(c))))throw new Error("\u7B7E\u5230\u5931\u8D25");t.toast("\u81EA\u52A8\u7B7E\u5230\u6210\u529F"),localStorage.setItem("signDate",d)}catch{t.toast.error("\u81EA\u52A8\u7B7E\u5230\u5931\u8D25")}})(),!!o.querySelector('a.post-list-cat-item[title="\u5728\u7EBF\u533A-\u6F2B\u753B"]')){if(o.querySelector(".content-hidden")){const s=o.querySelector(".content-hidden").getElementsByTagName("img");await o.wait(()=>s.length,1e3)&&n(()=>[...s].map(c=>c.src));return}if(o.querySelector(".xControl")){b.val=!1;const d=async s=>{l(s),r({onPrev:Reflect.has(h,s-1)?()=>d(s-1):void 0,onNext:Reflect.has(h,s+1)?()=>d(s+1):void 0})};for(const[s,c]of o.querySelectorAll(".xControl > a").entries()){const _=c.parentElement.nextElementSibling;n(()=>[..._.getElementsByTagName("img")].map(m=>m.dataset.src??""),s),c.addEventListener("click",()=>_.getAttribute("style")!==""&&d(s))}return}await o.wait(()=>o.querySelectorAll(".entry-content img").length),n(()=>o.querySelectorAll(".entry-content img").map(d=>d.dataset.src||d.src))}})();break}case"www.copy20.com":case"mangacopy.com":case"www.mangacopy.com":{const t=require("solid-js/web"),o=require("solid-js"),a=require("main"),r=require("helper"),n=new class{headers={webp:"1",region:"1","User-Agent":"COPY/2.0.7|",version:"2.0.7",source:"copyApp",referer:"com.copymanga.app-2.0.7"};get=async(d,s,...c)=>a.request(d,{responseType:"json",headers:this.headers,...s},...c)},l=new class{headers={"User-Agent":navigator.userAgent,referer:location.href};get=async(d,s,...c)=>a.request(`https://mapi.copy20.com${d}`,{responseType:"json",headers:this.headers,fetch:!1,...s},...c)},h=d=>{let s;const c=new CSSStyleSheet;document.adoptedStyleSheets.push(c);const _=async()=>{s||(async()=>{s=document.createElement("a");const g=await r.wait(()=>r.querySelector(".table-default-right"));s.target="_blank",g.insertBefore(s,g.firstElementChild);const w=document.createElement("span");w.textContent="\u6700\u5F8C\u95B1\u8B80\uFF1A",g.insertBefore(w,g.firstElementChild)})(),s.textContent="\u7372\u53D6\u4E2D",s.removeAttribute("href");const p=(await l.get(`/api/v3/comic2/${d}/query?platform=3`)).response?.results?.browse;if(!p){s.textContent=p===null?"\u7121":"\u672A\u8FD4\u56DE\u6578\u64DA";return}const k=p.chapter_id;if(!k){s.textContent="\u63A5\u53E3\u7570\u5E38";return}await c.replace(`ul a[href*="${k}"] {
        color: #fff !important;
        background: #1790E6;
      }`),s.href=`${window.location.pathname}/chapter/${k}`,s.textContent=p.chapter_name};setTimeout(_),document.addEventListener("visibilitychange",_)},b=async(d,s)=>{const{response:{results:c}}=await n.get(`/comicdetail/${d}/chapters`,{errorText:"\u52A0\u8F09\u6F2B\u756B\u76EE\u9304\u5931\u6557"}),m=await(async(v,f,$)=>{const L=await crypto.subtle.decrypt({name:"AES-CBC",iv:new TextEncoder().encode($)},await crypto.subtle.importKey("raw",new TextEncoder().encode(f),{name:"AES-CBC"},!1,["decrypt"]),new Uint8Array(v.match(/.{1,2}/g).map(M=>Number.parseInt(M,16))).buffer);return JSON.parse(new TextDecoder().decode(L))})(c.slice(16),unsafeWindow.dio||"xxymanga.zzl.key",c.slice(0,16));r.log(m);const{build:{type:p},groups:k}=m,g=v=>{const f=Object.fromEntries(p.map(({id:$})=>[$,[]]));for(const $ of v.chapters)f[$.type].push($);switch(s){case"mobile":{for(const $ of r.querySelectorAll(".van-divider"))$.remove();return(()=>{var $=t.template('<div class="detailsTextContentTabs van-tabs van-tabs--line">')();return t.insert($,t.createComponent(o.For,{each:p,children:({id:L,name:M})=>t.createComponent(o.Show,{get when(){return f[L].length},get children(){return[(()=>{var I=t.template('<div class=van-tabs__wrap><div role=tablist class="van-tabs__nav van-tabs__nav--line"><div role=tab class="van-tab van-tab--active"><span class="van-tab__text van-tab__text--ellipsis"><span></span></span></div><div class=van-tabs__line>')(),S=I.firstChild,u=S.firstChild,y=u.firstChild,x=y.firstChild,C=u.nextSibling;return S.style.setProperty("background","transparent"),t.insert(x,M),C.style.setProperty("width","0.24rem"),C.style.setProperty("transform","translateX(187.5px) translateX(-50%)"),C.style.setProperty("transition-duration","0.3s"),I})(),(()=>{var I=t.template('<div class=van-tab__pane><div class="chapterList van-grid">')(),S=I.firstChild;return S.style.setProperty("padding-left","0.24rem"),t.insert(S,t.createComponent(o.For,{get each(){return f[L]},children:u=>(()=>{var y=t.template('<div class="chapterItem oneLines van-grid-item"><a class="van-grid-item__content van-grid-item__content--center"><span class=van-grid-item__text>')(),x=y.firstChild,C=x.firstChild;return y.style.setProperty("flex-basis","25%"),y.style.setProperty("padding-right","0.24rem"),y.style.setProperty("margin-top","0.24rem"),t.insert(C,()=>u.name),t.effect(P=>{var T=v.last_chapter.uuid===u.id,A=`/comic/${d}/chapter/${u.id}`;return T!==P.e&&y.classList.toggle("red",P.e=T),A!==P.t&&t.setAttribute(x,"href",P.t=A),P},{e:void 0,t:void 0}),y})()})),I})()]}})})),$})()}case"web":return[(()=>{var $=t.template("<span>")();return t.insert($,()=>v.name),$})(),(()=>{var $=t.template('<div class=table-default><div class=table-default-title><ul class="nav nav-tabs"role=tablist></ul><div class=table-default-right><span>\u66F4\u65B0\u5167\u5BB9\uFF1A</span><a target=_blank></a><span>\u66F4\u65B0\u6642\u9593\uFF1A</span><span></span></div></div><div class=table-default-box><div class=tab-content>')(),L=$.firstChild,M=L.firstChild,I=M.nextSibling,S=I.firstChild,u=S.nextSibling,y=u.nextSibling,x=y.nextSibling,C=L.nextSibling,P=C.firstChild;return t.insert(M,t.createComponent(o.For,{each:p,children:({id:T,name:A})=>(()=>{var E=t.template("<li class=nav-item><a class=nav-link data-toggle=tab role=tab aria-selected=false>")(),D=E.firstChild;return t.insert(D,A),t.effect(N=>{var R=f[T].length===0,q=`#${v.path_word}${A}`;return R!==N.e&&D.classList.toggle("disabled",N.e=R),q!==N.t&&t.setAttribute(D,"href",N.t=q),N},{e:void 0,t:void 0}),E})()})),t.insert(u,()=>v.last_chapter.name),t.insert(x,()=>v.last_chapter.datetime_created),t.insert(P,t.createComponent(o.For,{each:p,children:({id:T,name:A})=>(()=>{var E=t.template('<div role=tabpanel class="tab-pane fade"><ul>')(),D=E.firstChild;return t.insert(D,t.createComponent(o.For,{get each(){return f[T]},children:N=>(()=>{var R=t.template("<a target=_blank><li>")(),q=R.firstChild;return R.style.setProperty("display","block"),t.insert(q,()=>N.name),t.effect(O=>{var z=`/comic/${d}/chapter/${N.id}`,F=N.name;return z!==O.e&&t.setAttribute(R,"href",O.e=z),F!==O.t&&t.setAttribute(R,"title",O.t=F),O},{e:void 0,t:void 0}),R})()})),t.effect(()=>t.setAttribute(E,"id",`${v.path_word}${A}`)),E})()})),t.effect(()=>t.setAttribute(u,"href",`/comic/${d}/chapter/${v.last_chapter.comic_id}`)),$})()];default:return t.createComponent(o.For,{each:p,children:({id:$,name:L})=>t.createComponent(o.Show,{get when(){return f[$].length},get children(){var M=t.template("<div class=card><div class=card-body><h2 class=card-title></h2><ul>")(),I=M.firstChild,S=I.firstChild,u=S.nextSibling;return M.style.setProperty("max-width","100em"),M.style.setProperty("margin","1em auto"),t.insert(S,L),t.insert(u,t.createComponent(o.For,{get each(){return f[$]},children:y=>(()=>{var x=t.template('<a class="btn btn-outline-primary">')();return t.insert(x,()=>y.name),t.effect(C=>{var P=v.last_chapter.uuid===y.id,T=`/comic/${d}/chapter/${y.id}`;return P!==C.e&&x.classList.toggle("active",C.e=P),T!==C.t&&t.setAttribute(x,"href",C.t=T),C},{e:void 0,t:void 0}),x})()})),M}})})}};let w;switch(s){case"mobile":w=r.querySelector(".detailsTextContent");for(const v of r.querySelectorAll("button.van-dialog__confirm"))v.click();break;case"web":w=r.querySelector(".upLoop");break;default:w=r.querySelector("main"),w.textContent="",GM_addStyle("ul .btn { height: fit-content; width: fit-content; margin: 1em; }");break}t.render(()=>t.createComponent(o.For,{get each(){return Object.values(k)},children:g}),w);for(const v of r.querySelectorAll(".upLoop .table-default-title"))v.querySelector(".nav-link:not(.disabled)")?.click()};(async()=>{const d=document.cookie.split("; ").find(_=>_.startsWith("token="))?.replace("token=","");d&&Reflect.set(n.headers,"Authorization",`Token ${d}`);let s="",c="";if(window.location.href.includes("/chapter/")?[,,s,,c]=window.location.pathname.split("/"):window.location.href.includes("/comicContent/")&&([,,,s,c]=window.location.pathname.split("/")),s&&c){const{setComicLoad:_,setManga:m}=await a.useInit("copymanga"),p=r.querySelector("main .img+.title");p&&(p.textContent="ComicRead \u63D0\u793A\u60A8\uFF1A\u4F60\u8A2A\u554F\u7684\u5167\u5BB9\u66AB\u4E0D\u5B58\u5728\uFF0C\u8ACB\u9EDE\u9078\u53F3\u4E0B\u89D2\u6309\u9215\u5617\u8A66\u52A0\u8F09\u6F2B\u756B"),_(async()=>{p&&(p.textContent="\u6F2B\u756B\u52A0\u8F09\u4E2D\uFF0C\u8ACB\u5750\u548C\u653E\u5BEC");const g=await l.get(`/api/v3/comic/${s}/chapter2/${c}?platform=3`,{noCheckCode:!0});if(g.status!==200){const $=`\u6F2B\u756B\u52A0\u8F09\u5931\u6557\uFF1A${g.response.message||g.status}`;throw p&&(p.textContent=$),new Error($)}if(p){p.textContent="\u6F2B\u756B\u52A0\u8F09\u6210\u529F\u{1F973}";const{chapter:{name:$},comic:{name:L}}=g.response.results;document.title=`${L} - ${$} - \u62F7\u8C9D\u6F2B\u756B \u62F7\u8D1D\u6F2B\u753B`}if(p??!r.querySelector(".comicContent-next")){const{chapter:{next:$,prev:L}}=g.response.results;m({onNext:$?()=>window.location.assign(`/comic/${s}/chapter/${$}`):void 0,onPrev:L?()=>window.location.assign(`/comic/${s}/chapter/${L}`):void 0})}else m({onNext:r.querySelectorClick(".comicContent-next a:not(.prev-null)"),onPrev:r.querySelectorClick(".comicContent-prev:not(.index,.list) a:not(.prev-null)")});const w=[],{words:v,contents:f}=g.response.results.chapter;for(let $=0;$<f.length;$++)w[v[$]]=f[$].url.replace(/(?<=.*(\/|\.))c800x/,"c1500x");return w}),m({commentList:await(async()=>{const g=window.location.pathname.split("/").at(-1);return(await l.get(`/api/v3/roasts?chapter_id=${g}&limit=100&offset=0&_update=true`,{errorText:"\u83B7\u53D6\u6F2B\u753B\u8BC4\u8BBA\u5931\u8D25"})).response.results.list.map(({comment:v})=>v)})()});return}if(!c&&window.location.href.includes("/comic/")){if(s=window.location.href.split("/comic/")[1],!s)return;let _;const m=window.location.href.includes("/h5/");if(document.title==="404 - \u62F7\u8C9D\u6F2B\u756B"?_=m?"mobile":"404":m?(await r.wait(()=>r.querySelector(".van-toast__text")?.parentElement?.style.display==="none"),_=await r.wait(()=>{if(r.querySelector(".isBan")?.textContent?.includes("\u4E0D\u63D0\u4F9B\u95B1\u89BD"))return"mobile";const p=r.querySelector(".van-dialog__message");if(p?.textContent?.includes("\u6F2B\u756B\u672A\u627E\u5230")){p.textContent=`\u6F2B\u756B\u672A\u627E\u5230!
\u8ACB\u5750\u548C\u653E\u5BEC\uFF0C\u7B49\u5F85\u76EE\u9304\u751F\u6210`;for(const k of r.querySelectorAll(".detailsTextContentTabs"))k.remove();return"mobile"}},1e3)):(r.querySelector(".wargin")?.textContent?.includes("\u4E0D\u63D0\u4F9B\u95B1\u89BD")||!await r.wait(()=>r.querySelector(".upLoop .table-default-title"),1e3))&&(_=r.querySelector(".comicParticulars-title")?"web":"404"),_){const p=r.querySelector(".isBan, .wargin");p&&(p.style.textDecoration="line-through");const k=r.querySelector("main .img+.title");k&&(k.textContent="ComicRead \u63D0\u793A\u60A8\uFF1A\u4F60\u8A2A\u554F\u7684\u5167\u5BB9\u66AB\u4E0D\u5B58\u5728\uFF0C\u8ACB\u5750\u548C\u653E\u5BEC\uFF0C\u7B49\u5F85\u76EE\u9304\u751F\u6210");try{await b(s,_)}catch{k&&(k.textContent="ComicRead \u63D0\u793A\u60A8\uFF1A\u76EE\u9304\u751F\u6210\u5931\u6557\u{1F622}"),a.toast.error("\u76EE\u9304\u751F\u6210\u5931\u6557\u{1F622}",{duration:Number.POSITIVE_INFINITY})}}!m&&d&&h(s)}})();break}case"www.pixiv.net":{let t=[];options={name:"pixiv",getImgList:()=>t,SPA:{async isMangaPage(){const o=Number(window.location.pathname.split("/")[2]);return!o||!window.location.pathname.startsWith("/artworks/")?(t.length=0,!1):(t=(await main.request(`/ajax/illust/${o}/pages`,{responseType:"json"})).response.body.map(r=>r.urls.original),t.length>1)}},initOptions:{autoShow:!1,defaultOption:{pageNum:1}}};break}case"www.ponpomu.com":{options={name:"terraHistoricus",wait:()=>!!helper.querySelector(".comic-page-container img"),getImgList:()=>helper.querySelectorAll(".comic-page-container img").map(t=>t.dataset.srcset),SPA:{isMangaPage:()=>window.location.href.includes("/comic/"),getOnPrev:()=>helper.querySelectorClick(".prev-btn:not(.invisible) a"),getOnNext:()=>helper.querySelectorClick(".next-btn:not(.invisible) a")}};break}case"manhua.zaimanhua.com":{options={name:"zaiManHua",wait:()=>!!helper.querySelector(".scrollbar-demo-item"),getImgList:()=>unsafeWindow.__NUXT__.data.getChapters?.data?.chapterInfo?.page_url,SPA:{isMangaPage:()=>window.location.pathname.startsWith("/view/"),getOnNext:()=>helper.querySelectorClick("#next_chapter"),getOnPrev:()=>helper.querySelectorClick("#prev_chapter")}};break}case"m.zaimanhua.com":{const t=async(a,r)=>{const n=await main.request(`https://v4api.zaimanhua.com/app/v1/comic/chapter/${a}/${r}`,{responseType:"json"});return n.response.errno&&main.toast.error(`${helper.t("alert.comic_load_error")}: ${n.response.errmsg}`,{throw:!0}),n.response.data.data},o=async a=>{const r=await main.request(`https://v4api.zaimanhua.com/app/v1/comic/detail/${a}`,{responseType:"json"});return r.response.errno&&main.toast.error(`${helper.t("alert.comic_load_error")}: ${r.response.errmsg}`,{throw:!0}),r.response.data.data};options={name:"zaiManHua",async getImgList({setManga:a}){const r=new URLSearchParams(window.location.search),n=Number(r.get("comic_id")),l=Number(r.get("chapter_id"));if(!n||!l)throw new Error(helper.t("site.changed_load_failed"));const h=await o(n),b=(h.chapters.length===1?h.chapters[0]:h.chapters.find(c=>c.data.find(_=>_.chapter_id===l))).data;b.sort((c,_)=>c.chapter_order-_.chapter_order);const d=b.findIndex(c=>c.chapter_id===l);return a({onPrev:d>0?()=>window.location.assign(`/pages/comic/page?comic_id=${n}&chapter_id=${b[d-1].chapter_id}`):void 0,onNext:d+1<b.length?()=>window.location.assign(`/pages/comic/page?comic_id=${n}&chapter_id=${b[d+1].chapter_id}`):void 0}),(await t(n,l)).page_url_hd},SPA:{isMangaPage:()=>window.location.pathname==="/pages/comic/page"}};break}case"terra-historicus.hypergryph.com":{const t=()=>`https://terra-historicus.hypergryph.com/api${window.location.pathname}`,o=a=>async()=>{const r=await main.request(`${t()}/page?pageNum=${a+1}`);return JSON.parse(r.responseText).data.url};options={name:"terraHistoricus",wait:()=>!!helper.querySelector(".HG_COMIC_READER_main"),async getImgList(){const r=(await main.request(t(),{responseType:"json"})).response.data.pageInfos;if(r.length===0&&window.location.pathname.includes("episode"))throw new Error("\u83B7\u53D6\u56FE\u7247\u5217\u8868\u65F6\u51FA\u9519");return helper.plimit(helper.createSequence(r.length).map(o))},SPA:{isMangaPage:()=>window.location.href.includes("episode"),getOnPrev:()=>helper.querySelectorClick("footer .HG_COMIC_READER_prev a"),getOnNext:()=>helper.querySelectorClick("footer .HG_COMIC_READER_prev+.HG_COMIC_READER_buttonEp a")}};break}case"jmcomic-zzz.one":case"jmcomic-zzz.org":case"18comic-cnye.club":case"18comic-cnye.org":case"18comic-xq.cc":case"18comic.org":case"18comic.vip":{const t=require("main"),o=require("helper");(async()=>{if(!window.location.pathname.includes("/photo/"))return;const{setComicLoad:a,setManga:r,dynamicLoad:n}=await t.useInit("jm");for(;!unsafeWindow?.onImageLoaded;){if(document.readyState==="complete"){t.toast.error("\u65E0\u6CD5\u83B7\u53D6\u56FE\u7247",{duration:Number.POSITIVE_INFINITY});return}await o.sleep(100)}r({onPrev:o.querySelectorClick(()=>o.querySelector(".menu-bolock-ul .fa-angle-double-left")?.parentElement),onNext:o.querySelectorClick(()=>o.querySelector(".menu-bolock-ul .fa-angle-double-right")?.parentElement)});const l=o.querySelectorAll(".scramble-page:not(.thewayhome) > img");if(unsafeWindow.aid<unsafeWindow.scramble_id||unsafeWindow.speed==="1")return a(()=>l.map(s=>s.dataset.original??""));const h=async s=>{try{return await t.request(s,{responseType:"blob",fetch:!0,noTip:!0},3)}catch{return await t.request(s,{responseType:"blob",revalidate:!0,fetch:!1},3)}},b=async s=>{if(s.src.startsWith("blob:"))return s.src;const c=s.dataset.original,_=await h(s.dataset.original);if(_.response.size===0)return t.toast.warn(`\u4E0B\u8F7D\u539F\u56FE\u65F6\u51FA\u9519: ${s.dataset.page}`),"";s.src=`${URL.createObjectURL(_.response)}#${s.src}`;try{await o.waitImgLoad(s,1e3*10)}catch{return URL.revokeObjectURL(s.src),s.src=c,t.toast.warn(`\u52A0\u8F7D\u539F\u56FE\u65F6\u51FA\u9519: ${s.dataset.page}`),""}try{s.nextElementSibling?.tagName==="CANVAS"&&s.nextElementSibling.remove(),unsafeWindow.onImageLoaded(s);const m=await o.canvasToBlob(s.nextElementSibling,"image/webp",1);if(URL.revokeObjectURL(s.src),!m)throw new Error("\u8F6C\u6362\u56FE\u7247\u65F6\u51FA\u9519");return`${URL.createObjectURL(m)}#.webp`}catch(m){return s.src=c,t.toast.warn(`\u8F6C\u6362\u56FE\u7247\u65F6\u51FA\u9519: ${s.dataset.page}, ${m.message}`),""}};await o.wait(()=>{const s=o.querySelectorAll(".lazy-loaded").length;return s>0&&o.querySelectorAll("canvas").length-s<=1}),a(n(async s=>o.plimit(l.map((c,_)=>async()=>{s(_,await b(c))})),l.length))})().catch(a=>o.log.error(a));break}case"tw.manhuagui.com":case"m.manhuagui.com":case"www.mhgui.com":case"www.manhuagui.com":{if(!/\/comic\/\d+\/\d+\.html/.test(window.location.pathname))break;let comicInfo;try{const dataScript=helper.querySelectorAll("body > script:not([src])").find(t=>t.innerHTML.startsWith("window["));if(!dataScript)throw new Error(helper.t("site.changed_load_failed"));comicInfo=JSON.parse(eval(dataScript.innerHTML.slice(26)).match(/(?<=.*?\(){.+}/)[0])}catch{main.toast.error(helper.t("site.changed_load_failed"));break}GM_addStyle("#smh-msg-box { z-index: 2147483647 !important }");const handlePrevNext=t=>{if(t===0)return;const o=window.location.pathname.replace(/(?<=\/)\d+(?=\.html)/,`${t}`);return()=>window.location.assign(o)};options={name:"manhuagui",getImgList(){const t=Object.entries(comicInfo.sl).map(o=>`${o[0]}=${o[1]}`).join("&");if(comicInfo.files)return comicInfo.files.map(o=>`${unsafeWindow.pVars.manga.filePath}${o}?${t}`);if(comicInfo.images){const{origin:o}=new URL(helper.querySelector("#manga img").src);return comicInfo.images.map(a=>`${o}${a}?${t}`)}return main.toast.error(helper.t("site.changed_load_failed"),{throw:!0}),[]},onNext:handlePrevNext(comicInfo.nextId),onPrev:handlePrevNext(comicInfo.prevId)};break}case"www.manhuadb.com":{if(!Reflect.has(unsafeWindow,"img_data_arr"))break;options={name:"manhuaDB",getImgList:()=>unsafeWindow.img_data_arr.map(t=>`${unsafeWindow.img_host}/${unsafeWindow.img_pre}/${t.img}`),onPrev:()=>unsafeWindow.goNumPage("pre"),onNext:()=>unsafeWindow.goNumPage("next")};break}case"www.manhuaren.com":case"m.1kkk.com":case"www.1kkk.com":case"tel.dm5.com":case"en.dm5.com":case"cnc.dm5.com":case"www.dm5.cn":case"www.dm5.com":{if(!Reflect.has(unsafeWindow,"DM5_CID"))break;const imgNum=unsafeWindow.DM5_IMAGE_COUNT??unsafeWindow.imgsLen;if(!(Number.isSafeInteger(imgNum)&&imgNum>0)){main.toast.error(helper.t("site.changed_load_failed"));break}const getPageImg=async i=>{const res=await unsafeWindow.$.ajax({type:"GET",url:"chapterfun.ashx",data:{cid:unsafeWindow.DM5_CID,page:i,key:unsafeWindow.$("#dm5_key").length>0?unsafeWindow.$("#dm5_key").val():"",language:1,gtk:6,_cid:unsafeWindow.DM5_CID,_mid:unsafeWindow.DM5_MID,_dt:unsafeWindow.DM5_VIEWSIGN_DT,_sign:unsafeWindow.DM5_VIEWSIGN}});return eval(res)},handlePrevNext=(t,o)=>helper.querySelectorClick(()=>helper.querySelector(t)??helper.querySelectorAll(".view-bottom-bar a").find(a=>a.textContent?.includes(o)));options={name:"dm5",getImgList({dynamicLoad:t}){return Array.isArray(unsafeWindow.newImgs)&&unsafeWindow.newImgs.every(helper.isUrl)?unsafeWindow.newImgs:t(async o=>{const a=new Set;for(;a.size<imgNum;)for(const r of await getPageImg(a.size+1))a.has(r)||(a.add(r),o(a.size-1,r))},imgNum)()},onPrev:handlePrevNext(".logo_1","\u4E0A\u4E00\u7AE0"),onNext:handlePrevNext(".logo_2","\u4E0B\u4E00\u7AE0"),onExit:t=>t&&helper.scrollIntoView(".postlist")};break}case"www.wnacg02.cc":case"www.wnacg01.cc":case"www.wn03.ru":case"www.wnacg.com":case"wnacg.com":{const t=helper.querySelector("#bodywrap a.btn");if(t&&(t.style.setProperty("background-color","#607d8b"),t.style.setProperty("background-image","none")),!Reflect.has(unsafeWindow,"imglist"))break;options={name:"wnacg",getImgList:()=>unsafeWindow.imglist.filter(({caption:o})=>o!=="\u559C\u6B61\u7D33\u58EB\u6F2B\u756B\u7684\u540C\u5B78\u8ACB\u52A0\u5165\u6536\u85CF\u54E6\uFF01").map(({url:o})=>new URL(o,window.location.origin).href)};break}case"www.mangabz.com":case"mangabz.com":{if(!Reflect.has(unsafeWindow,"MANGABZ_CID"))break;const imgNum=unsafeWindow.MANGABZ_IMAGE_COUNT??unsafeWindow.imgsLen;if(!(Number.isSafeInteger(imgNum)&&imgNum>0)){main.toast.error(helper.t("site.changed_load_failed"));break}const getPageImg=async i=>{const res=await unsafeWindow.$.ajax({type:"GET",url:"chapterimage.ashx",data:{cid:unsafeWindow.MANGABZ_CID,page:i,key:"",_cid:unsafeWindow.MANGABZ_CID,_mid:unsafeWindow.MANGABZ_MID,_dt:unsafeWindow.MANGABZ_VIEWSIGN_DT,_sign:unsafeWindow.MANGABZ_VIEWSIGN}});return eval(res)},handlePrevNext=(t,o)=>helper.querySelectorClick(()=>helper.querySelector(t)??helper.querySelectorAll(".bottom-bar-tool a").find(a=>a.textContent?.includes(o)));options={name:"mangabz",getImgList:({dynamicLoad:t})=>t(async o=>{const a=new Set;for(;a.size<imgNum;)for(const r of await getPageImg(a.size+1))a.has(r)||(a.add(r),o(a.size-1,r))},imgNum)(),onNext:handlePrevNext('body > .container a[href^="/"]:last-child',"\u4E0B\u4E00"),onPrev:handlePrevNext('body > .container a[href^="/"]:first-child',"\u4E0A\u4E00")};break}case"komiic.com":{const t=`
