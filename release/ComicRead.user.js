...
worker.setMainFn(Comlink.proxy(mainFn$1), Object.keys(mainFn$1));
...
const getModel = async () => {
  ...
  try {
    ...
    helper.log.error('\u5207\u6362 WebGPU \u51FA\u9519', error);
  }
  const {
    buffer,
    base64,
...
`;break;default:o=getResource(t)}if(t.startsWith("worker/")&&supportWorker)try{const n=new Map;n.set("Comlink",getResource("comlink"));const r=c=>c.replaceAll(/require\('(.+?)'\)/g,(d,l)=>(n.has(l)||n.set(l,r(getResource(l))),`moduleMap['${l}']`));let s=`const moduleMap = {};
`;for(const[c,d]of n)s+=`
moduleMap['${c}'] = {};
(function (exports, module) { ${d} }) (
  moduleMap['${c}'],
  {
    set exports(value) { moduleMap['${c}'] = value; },
    get exports() { return moduleMap['${c}']; }
  },
);
`;s+=`
const exports = {};
${r(o)}
moduleMap['Comlink'].expose(exports);`;const _=URL.createObjectURL(new Blob([s],{type:"text/javascript"}));setTimeout(URL.revokeObjectURL,0,_);const p=new Worker(_);crsLib[t]=require("comlink").wrap(p);return}catch{supportWorker=!1}let a=`
  ...
  `;unsafeWindow[tempName]=crsLib,unsafeWindow[tempName][t]={},evalCode(a),Reflect.deleteProperty(unsafeWindow,tempName)},require=t=>{const o={value:!0},a=()=>{};a.default={};const n=new Proxy(a,{get(r,s){return s==="__esModule"?o:s==="default"?n:(crsLib[t]||selfImportSync(t),Reflect.has(crsLib[t],"default")&&Reflect.has(crsLib[t].default,s)?crsLib[t].default[s]:crsLib[t][s])},apply(r,s,_){crsLib[t]||selfImportSync(t);const p=crsLib[t];return(typeof p.default=="function"?p.default:p)(..._)},construct(r,s){crsLib[t]||selfImportSync(t);const _=crsLib[t],p=typeof _.default=="function"?_.default:_;return new p(...s)}});return n};crsLib.require=require;const Manga=require("components/Manga"),helper=require("helper"),languages=require("helper/languages"),main=require("main"),otherSite=require("userscript/otherSite");let options;try{switch(location.hostname){case"bbs.yamibo.com":{const t=require("solid-js/web"),o=require("solid-js"),a=require("helper"),n=require("main");(async()=>{const{setState:r,options:s,showComic:_,loadComic:p}=await n.useInit("yamibo",{\u8BB0\u5F55\u9605\u8BFB\u8FDB\u5EA6:!0,\u5173\u95ED\u5FEB\u6377\u5BFC\u822A\u7684\u8DF3\u8F6C:!0,\u4FEE\u6B63\u70B9\u51FB\u9875\u6570\u65F6\u7684\u8DF3\u8F6C\u5224\u5B9A:!0,\u56FA\u5B9A\u5BFC\u822A\u6761:!0,\u81EA\u52A8\u7B7E\u5230:!0});if(a.useStyle(`#fab { --fab: #6E2B19; }
    ...
    `),unsafeWindow.discuz_uid&&unsafeWindow.discuz_uid!=="0"&&s.\u81EA\u52A8\u7B7E\u5230&&(async()=>{const c=new Date().toLocaleDateString("zh-CN");if(c===localStorage.getItem("signDate"))return;const d=a.querySelector('#scbar_form > input[name="formhash"]')?.value;if(d)try{const m=await(await fetch(`plugin.php?id=zqlj_sign&sign=${d}`)).text();if(!/成功！|打过卡/.test(m))throw new Error("\u81EA\u52A8\u7B7E\u5230\u5931\u8D25");n.toast.success("\u81EA\u52A8\u7B7E\u5230\u6210\u529F"),localStorage.setItem("signDate",c)}catch{n.toast.error("\u81EA\u52A8\u7B7E\u5230\u5931\u8D25")}})(),s.\u5173\u95ED\u5FEB\u6377\u5BFC\u822A\u7684\u8DF3\u8F6C&&a.querySelector("#qmenu a")?.setAttribute("href","javascript:;"),/thread(?:-\d+){3}|mod=viewthread/.test(document.URL)){for(const l of a.querySelectorAll('img[file*="sinaimg.cn"]'))l.setAttribute("referrerpolicy","no-referrer");const c=()=>{!a.querySelector(".pg > .prev")||r("flag","needAutoShow",!1);let m=a.querySelectorAll(":is(.t_fsz, .message) img");const f=()=>{let b=m.length;for(;b--;){const u=m[b],w=u.getAttribute("file");w&&u.src!==w&&(u.setAttribute("src",w),u.setAttribute("lazyloaded","true")),(u.src.includes("static/image")||u.complete&&u.naturalHeight&&u.naturalWidth&&u.naturalHeight<500&&u.naturalWidth<500)&&m.splice(b,1)}return m.map(u=>u.src)};r("comicMap","",{getImgList:f}),r("manga",{onLoading(b,u){if(u&&u.width<500&&u.height<500)return p()},onExit(b){b&&a.scrollIntoView(".psth, .rate, #postlist > div:nth-of-type(2)"),r("manga","show",!1)}}),a.querySelector("div.pti > div.authi")&&(a.querySelector("div.pti > div.authi").insertAdjacentHTML("beforeend",'<span class="pipe show">|</span><a id="comicReadMode" class="show" href="javascript:;">\u6F2B\u753B\u9605\u8BFB</a>'),document.getElementById("comicReadMode")?.addEventListener("click",()=>_())),a.querySelector("#threadindex")&&a.hijackFn("ajaxinnerhtml",()=>{m=a.querySelectorAll(".t_fsz img"),!(m.length===0||f().length===0)&&s.autoShow&&_()});const h=a.querySelector(".ptg.mbm.mtn > a");if(h){const[,b]=h.href.split("id="),u=/(?<=<th>\s<a href="thread-)\d+(?=-)/g;let w=[];const S=async(x=1)=>{const v=[...(await n.request(`/misc.php?mod=tag&id=${b}&type=thread&page=${x}`)).responseText.matchAll(u)].map(([g])=>Number(g));w=[...w,...v];const y=w.indexOf(unsafeWindow.tid);return v.length>0&&(y===-1||!w[y+1])?S(x+1):r("manga",{onPrev:w[y-1]?()=>location.assign(`thread-${w[y-1]}-1-1.html`):void 0,onNext:w[y+1]?()=>location.assign(`thread-${w[y+1]}-1-1.html`):void 0})};setTimeout(S)}},d=unsafeWindow.fid??Number(new URLSearchParams(a.querySelector("h2 > a")?.href).get("fid")??"-1");if(d===30||d===37)c();else{a.querySelector("div.pti > div.authi").insertAdjacentHTML("beforeend",'<span class="pipe show">|</span><a id="comicReadMode" class="show" href="javascript:;">\u6F2B\u753B\u9605\u8BFB</a>');const l=document.getElementById("comicReadMode");l?.addEventListener("click",()=>{l.previousElementSibling?.remove(),l.remove(),c(),_()})}if(s.\u8BB0\u5F55\u9605\u8BFB\u8FDB\u5EA6){const l=unsafeWindow.tid??new URLSearchParams(location.search).get("tid")??/\/thread-(\d+)-\d+-\d+.html/.exec(location.pathname)?.[1];if(!l)return;let m;try{const $=await n.request(`/api/mobile/index.php?module=viewthread&tid=${l}`,{responseType:"json",errorText:"\u83B7\u53D6\u5E16\u5B50\u56DE\u590D\u6570\u65F6\u51FA\u9519",noTip:!0});m=Number.parseInt($.response?.Variables?.thread?.allreplies,10)}catch{}const f=Number.parseInt(a.querySelector("#pgt strong")?.textContent??a.querySelector("#dumppage")?.value??"1",10),h=await a.useCache({history:"tid"}),b=await h.get("history",`${l}`);if(b&&f<b.lastPageNum)return;const u=a.querySelectorAll(b?.lastAnchor&&f===b.lastPageNum?`#${b.lastAnchor} ~ div`:"#postlist > div, .plc.cl");if(u.length===0)return;let w=0;const S=$=>{w&&window.clearTimeout(w),w=window.setTimeout(async()=>{w=0,await h.set("history",$)},200)},x=new IntersectionObserver($=>{const v=$.find(g=>g.isIntersecting);if(!v)return;const y=u.indexOf(v.target);if(y!==-1){for(const g of u.splice(0,y+1))x.unobserve(g);S({tid:`${l}`,lastPageNum:f,lastReplies:m||b?.lastReplies||0,lastAnchor:v.target.id})}},{rootMargin:"-160px"});for(const $ of u)x.observe($)}return}if(/forum(?:-\d+){2}|mod=forumdisplay/.test(document.URL)){if(s.\u4FEE\u6B63\u70B9\u51FB\u9875\u6570\u65F6\u7684\u8DF3\u8F6C\u5224\u5B9A){const c=a.querySelectorAll(".tps>a");let d=c.length;for(;d--;)c[d].setAttribute("onClick","atarget(this)")}if(s.\u8BB0\u5F55\u9605\u8BFB\u8FDB\u5EA6){const c=await a.useCache({history:"tid"}),d=!document.querySelector("#flk"),[l,m]=o.createSignal(!1),f=()=>m(w=>!w);let h="tbody[id^=normalthread]",b=w=>w.id.split("_")[1],u=(w,S)=>`thread-${S}-${w.lastPageNum}-1.html#${w.lastAnchor}`;d&&(h=".threadlist li.list",b=w=>new URLSearchParams(w.children[1].getAttribute("href")).get("tid"),u=(w,S)=>`forum.php?mod=viewthread&tid=${S}&extra=page%3D1&mobile=2&page=${w.lastPageNum}#${w.lastAnchor}`);for(const w of a.querySelectorAll(h)){const S=b(w);t.render(()=>{const[x,$]=o.createSignal();a.createEffectOn(l,()=>c.get("history",S).then($));const v=o.createMemo(()=>x()?u(x(),S):""),y=o.createMemo(()=>!d&&x()?Number(w.querySelector(".num a").innerHTML)-x().lastReplies:0),g=()=>[(()=>{var I=t.template("<a class=historyTag>\u56DE\u7B2C<!>\u9875 ")(),L=I.firstChild,C=L.nextSibling;return t.addEventListener(I,"click",unsafeWindow.atarget,!0),t.insert(I,()=>x()?.lastPageNum,C),t.effect(()=>t.setAttribute(I,"href",v())),I})(),t.createComponent(o.Show,{get when(){return y()>0},get children(){var I=t.template("<div class=historyTag>+")();return t.insert(I,y,null),I}})],k=()=>(()=>{var I=t.template("<li><a>\u56DE\u7B2C<!>\u9875")(),L=I.firstChild,C=L.firstChild,M=C.nextSibling;return t.addEventListener(L,"click",unsafeWindow.atarget,!0),t.setStyleProperty(L,"color","unset"),t.insert(L,()=>x()?.lastPageNum,M),t.effect(()=>t.setAttribute(L,"href",v())),I})();return t.createComponent(o.Show,{get when(){return!!x()},get children(){return t.createComponent(o.Show,{when:d,get children(){return k()},get fallback(){return g()}})}})},d?w.children[3]:w.getElementsByTagName("th")[0])}document.addEventListener("visibilitychange",f),a.querySelector("#autopbn")?.addEventListener("click",f)}}})().catch(r=>a.log.error(r)),t.delegateEvents(["click"]);break}case"www.yamibo.com":{if(!location.pathname.includes("/manga/view-chapter"))break;const t=new URLSearchParams(location.search).get("id");if(!t)break;const o=Number(helper.querySelector("section div:first-of-type div:last-of-type").innerHTML.split("\uFF1A")[1]);if(Number.isNaN(o))throw new Error(helper.t("site.changed_load_failed"));const a=async r=>{const s=await main.request(`https://www.yamibo.com/manga/view-chapter?id=${t}&page=${r}`);return/(?<=<img id=['"]imgPic['"].+?src=['"]).+?(?=['"])/.exec(s.responseText)[0].replaceAll("&amp;","&").replaceAll("http://","https://")},n=r=>helper.plimit(helper.range(o,s=>async()=>r(s,await a(s+1))));options={name:"newYamibo",getImgList:({dynamicLoad:r})=>r(n,o),onNext:helper.querySelectorClick("#btnNext"),onPrev:helper.querySelectorClick("#btnPrev"),onExit:r=>r&&helper.scrollIntoView("#w1")};break}case"comic.idmzj.com":case"comic.dmzj.com":case"manhua.idmzj.com":case"manhua.dmzj.com":{const t=require("solid-js/web"),o=require("solid-js"),a=require("helper"),n=require("main"),r=require("userscript/dmzjApi");(async()=>{const s=async()=>{const[,m,f]=location.pathname.split(/\/|\./);return m||n.toast.error("\u6F2B\u753B\u6570\u636E\u83B7\u53D6\u5931\u8D25",{duration:Number.POSITIVE_INFINITY,throw:new Error("\u83B7\u53D6\u6F2B\u753B\u62FC\u97F3\u7B80\u79F0\u5931\u8D25")}),{comicId:await r.getComicId(m),chapterId:f}},_=async()=>{if(await a.waitDom(".commentBox"),!a.querySelector(".cartoon_online_border > img"))return!1;a.querySelector(".cartoon_online_border").innerHTML="\u83B7\u53D6\u6F2B\u753B\u6570\u636E\u4E2D";for(const f of a.querySelectorAll(".odd_anim_title ~ *"))f.remove();const{comicId:m}=await s();return t.render(()=>{const f=r.useComicDetail(m);return t.createComponent(o.For,{get each(){return f.chapters},children:({name:h,list:b})=>[(()=>{var u=t.template('<div class=photo_part><div class=h2_title2><span class="h2_icon h2_icon22"></span><h2> ')(),w=u.firstChild,S=w.firstChild,x=S.nextSibling,$=x.firstChild;return t.insert(x,()=>f.title,$),t.insert(x,h==="\u8FDE\u8F7D"?"\u5728\u7EBF\u6F2B\u753B\u5168\u96C6":`\u6F2B\u753B\u5176\u5B83\u7248\u672C\uFF1A${h}`,null),u})(),(()=>{var u=t.template("<div class=cartoon_online_border_other style=margin-top:-8px><ul></ul><div class=clearfix>")(),w=u.firstChild;return t.insert(w,t.createComponent(o.For,{each:b,children:({title:S,id:x,updatetime:$})=>(()=>{var v=t.template("<li><a target=_blank>")(),y=v.firstChild;return t.setAttribute(y,"title",S),t.setAttribute(y,"href",`https://m.dmzj.com/view/${m}/${x}.html`),t.insert(y,S),t.effect(()=>y.classList.toggle("color_red",$===f.last_updatetime)),v})()})),u})()]})},a.querySelector(".middleright_mr")),!1},p=async()=>{await a.waitDom("#qiehuan_txt"),await a.wait(()=>{const m=a.querySelector("#qiehuan_txt");if(m){if(m.textContent!=="\u5207\u6362\u5230\u4E0A\u4E0B\u6EDA\u52A8\u9605\u8BFB")return!0;m.click()}})},c=async()=>(await p(),await a.waitDom(".comic_wraCon img"),a.querySelectorAll(".comic_wraCon img").map(m=>m.src)),d=m=>{const f=a.querySelector(m);if(f?.textContent)return()=>f.click()},l=()=>/^\/[^/]*\/?$/.test(location.pathname)?_():/^\/.*?\/\d+\.shtml$/.test(location.pathname);await n.universal({name:"dmzj",getImgList:c,onExit:m=>m&&a.scrollIntoView("#hd"),async getCommentList(){const{comicId:m,chapterId:f}=await s();return r.getViewpoint(m,f)},SPA:{isMangaPage:l,getOnPrev:()=>d(".display_left #prev_chapter"),getOnNext:()=>d(".display_right #next_chapter")}})})().catch(s=>a.log.error(s));break}case"m.idmzj.com":case"m.dmzj.com":{const t=require("dmzjDecrypt"),o=require("helper"),a=require("main"),n=require("userscript/dmzjApi");(async()=>{switch(location.pathname.split("/")[1]){case"info":{if(Reflect.has(unsafeWindow,"obj_id"))return;const r=Number.parseInt(location.pathname.split("/")[2],10);if(Number.isNaN(r)){document.body.innerHTML="",document.body.insertAdjacentHTML("beforeend",`
            \u8BF7\u624B\u52A8\u8F93\u5165\u6F2B\u753B\u540D\u8FDB\u884C\u641C\u7D22 <br />
            <input type="search"> <button>\u641C\u7D22</button> <br />
            <div id="list" />
          `),o.querySelector("button").addEventListener("click",async()=>{const d=o.querySelector("input")?.value;if(!d)return;const l=await a.request(`https://s.acg.dmzj.com/comicsum/search.php?s=${d}`,{errorText:"\u641C\u7D22\u6F2B\u753B\u65F6\u51FA\u9519"}),m=JSON.parse(l.responseText.slice(20,-1));o.querySelector("#list").innerHTML=m.map(({id:f,comic_name:h,comic_author:b,comic_url:u})=>`
                <b>\u300A${h}\u300B<b/>\u2014\u2014${b}
                <a href="${u}">Web\u7AEF</a>
                <a href="https://m.dmzj.com/info/${f}.html">\u79FB\u52A8\u7AEF</a>
              `).join("<br />")});return}const s=await a.request(`https://v4api.idmzj.com/comic/detail/${r}?uid=2665531&disable_level=1`,{errorText:"\u83B7\u53D6\u6F2B\u753B\u6570\u636E\u5931\u8D25"}),{comicInfo:{last_updatetime:_,title:p,chapters:c}}=t(s.responseText);document.title=p,document.body.insertAdjacentHTML("beforeend",`<h1>${p}</h1>`);for(const d of Object.values(c)){let l=`<h2>${d.title}</h2>`,m=d.data.length;for(;m--;)l+=`<a target="_blank" title="${d.data[m].chapter_title}" href="https://m.dmzj.com/view/${r}/${d.data[m].chapter_id}.html" ${d.data[m].updatetime===_?'style="color:red"':""}>${d.data[m].chapter_title}</a>`;document.body.insertAdjacentHTML("beforeend",l)}document.body.childNodes[0].remove(),o.useStyle(`
          h1 { margin: 0 -20vw; }
        ...
        `);break}case"view":{if(unsafeWindow.comic_id){o.useStyle(".subHeader{display:none !important}"),await a.universal({name:"dmzj",getImgList:()=>o.querySelectorAll("#commicBox img").map(u=>u.dataset.original).filter(Boolean),getCommentList:()=>n.getViewpoint(unsafeWindow.subId,unsafeWindow.chapterId),onNext:o.querySelectorClick("#loadNextChapter"),onPrev:o.querySelectorClick("#loadPrevChapter")});return}const r=document.createElement("p");r.textContent="\u6B63\u5728\u52A0\u8F7D\u4E2D\uFF0C\u8BF7\u5750\u548C\u653E\u5BBD\uFF0C\u82E5\u957F\u65F6\u95F4\u65E0\u53CD\u5E94\u8BF7\u5237\u65B0\u9875\u9762",document.body.append(r);let s,_,p;try{[,_,p]=/(\d+)\/(\d+)/.exec(location.pathname),s=await n.getChapterInfo(_,p)}catch(u){throw a.toast.error("\u83B7\u53D6\u6F2B\u753B\u6570\u636E\u5931\u8D25",{duration:Number.POSITIVE_INFINITY}),r.textContent=u.message,u}r.textContent="\u52A0\u8F7D\u5B8C\u6210\uFF0C\u5373\u5C06\u8FDB\u5165\u9605\u8BFB\u6A21\u5F0F";const{folder:c,chapter_name:d,next_chap_id:l,prev_chap_id:m,comic_id:f,page_url:h}=s;document.title=`${d} ${c.split("/").at(1)}`;const{setState:b}=await a.useInit("dmzj");b(u=>{u.manga.onExit=void 0,u.manga.onNext=l?()=>{location.href=`https://m.dmzj.com/view/${f}/${l}.html`}:void 0,u.manga.onPrev=m?()=>{location.href=`https://m.dmzj.com/view/${f}/${m}.html`}:void 0,u.manga.editButtonList=w=>w,u.comicMap[""].getImgList=()=>h.length>0?h:(r.innerHTML='\u65E0\u6CD5\u83B7\u5F97\u6F2B\u753B\u6570\u636E\uFF0C\u8BF7\u901A\u8FC7 <a href="https://github.com/hymbz/ComicReadScript/issues" target="_blank">Github</a> \u6216 <a href="https://sleazyfork.org/zh-CN/scripts/374903-comicread/feedback#post-discussion" target="_blank">Greasy Fork</a> \u8FDB\u884C\u53CD\u9988',[])}),b("manga","commentList",await n.getViewpoint(_,p));break}}})().catch(r=>o.log.error(r));break}case"www.idmzj.com":case"www.dmzj.com":{const t=require("helper"),o=require("main"),a=require("userscript/dmzjApi"),n=r=>{if(r)return()=>{window.open(location.href.replace(/(?<=\/)\d+(?=\.html)/,`${r}`),"_self")}};(async()=>{await t.waitDom(".head_wz");const r=t.querySelector(".head_wz [id]")?.id,s=/(?<=\/)\d+(?=\.html)/.exec(location.pathname)?.[0];if(!r||!s)return;const{setState:_}=await o.useInit("dmzj");try{const{next_chap_id:p,prev_chap_id:c,page_url:d}=await a.getChapterInfo(r,s);_(l=>{l.comicMap[""].getImgList=()=>d,l.manga.onNext=n(p),l.manga.onPrev=n(c)})}catch{o.toast.error("\u83B7\u53D6\u6F2B\u753B\u6570\u636E\u5931\u8D25",{duration:Number.POSITIVE_INFINITY})}})().catch(r=>t.log.error(r));break}case"exhentai.org":case"e-hentai.org":{const web=require("solid-js/web"),solidJs=require("solid-js"),Manga=require("components/Manga"),helper=require("helper"),main=require("main"),store=require("solid-js/store"),detectAd$1=require("userscript/detectAd"),ehTagRules=require("userscript/ehTagRules"),getTagSetHtml=async t=>{const o=t?`/mytags?tagset=${t}`:"/mytags",a=await main.request(o,{fetch:!0});return helper.domParse(a.responseText)},collectTags=(t,o=[])=>{const a=t.querySelector("#tagcolor").value.slice(1)||"0",[,...n]=[...t.getElementById("usertags_outer").children];for(const r of n){const s=Number(r.id.split("usertag_")[1]),_=r.querySelector(`#tagpreview_${s}`),{color:p,borderColor:c}=_.style;let[d,l]=_.title.split(":");switch(d){case"female":case"male":case"mixed":d="gender"}const m=Number.parseInt(r.querySelector(`#tagcolor_${s}`).value.slice(1)||a,16);o.push({e:r,id:s,title:_.title,color:m,fontColor:p,borderColor:c,group:d,name:l,weight:Number(r.querySelector("input[id^=tagweight_]").value),watch:r.querySelector(`#tagwatch_${s}`).checked,hidden:r.querySelector(`#taghide_${s}`).checked,order:-1})}return o},sortTagList=t=>{const o=new Intl.Collator,a=(r,s)=>r.color!==s.color?s.color-r.color:r.group!==s.group?o.compare(r.group,s.group):r.hidden!==s.hidden?r.hidden?1:-1:r.watch!==s.watch?r.watch?-1:1:r.weight!==s.weight?s.weight-r.weight:o.compare(r.name,s.name);let n=-t.length;for(const r of t.sort(a))r.order=n++;return t},getMyTags=async()=>{const t=[],o=await getTagSetHtml();await Promise.all([...o.querySelectorAll("#tagset_outer select option")].map(async n=>{const r=n.selected?o:await getTagSetHtml(n.value);r.querySelector("#tagset_enable")?.checked&&t.push(r)}));const a=[];for(const n of t)collectTags(n,a);return sortTagList(a)},handleMyTagsChange=new Set,updateMyTags=async()=>{const t=await getMyTags();for(const o of handleMyTagsChange)await o(t)},buildTagList=(t,o)=>`
${[...t].map(a=>`${o}${CSS.escape(a)}`).join(`,
`)}
`,updateTagColor=async t=>{const o={},a={},n={};for(const s of t){const{color:_,borderColor:p,fontColor:c}=s,d=s.title.replaceAll(" ","_");(o[_]||=new Set).add(d),(a[p]||=new Set).add(d),(n[c]||=new Set).add(d)}let r="";for(const[s,_]of Object.entries(o))r+=`:is(${buildTagList(_,"#td_")})`,r+=`{ background: #${Number(s).toString(16).padStart(6,"0")}; }

`;for(const[s,_]of Object.entries(a))r+=`:is(${buildTagList(_,"#td_")}).gt`,r+=`{ border-color: ${s}; }

`;for(const[s,_]of Object.entries(n))r+=`:is(${buildTagList(_,"#td_")}):not(.gt)`,r+=`{ border-color: ${s}; }

`,r+=`#taglist a:is(${buildTagList(_,"#ta_")})`,r+=`{ color: ${s} !important; position: relative; }
...
`,o+=await helper.getGmValue("ehTagColorizeCss",updateMyTags),helper.useStyle(o)}case"mytags":updateMyTags(),helper.hijackFn("usertag_callback",helper.debounce(updateMyTags))}},nhentai=async({setState:t,galleryTitle:o})=>{const{response:{result:a}}=await main.request(`https://nhentai.net/api/galleries/search?query=${o}`,{responseType:"json",errorText:helper.t("site.ehentai.nhentai_error"),noTip:!0,headers:{"User-Agent":navigator.userAgent},fetch:!1}),n=async(r,s,_)=>{const p=await main.request(`https://i.nhentai.net/galleries/${s}/${r+1}.${helper.fileType[_]}`,{headers:{Referer:`https://nhentai.net/g/${s}`},responseType:"blob",fetch:!1});return URL.createObjectURL(p.response)};return a.map(({id:r,title:s,images:_,num_pages:p,media_id:c})=>{const d=`@nh:${r}`;return t("comicMap",d,{getImgList:({dynamicLoad:l})=>l(m=>helper.plimit(_.pages.map((f,h)=>async()=>m(h,await n(h,c,f.t)))),p,d)}),{id:d,showText:`${r}`,title:s.english||s.japanese,href:`https://nhentai.net/g/${r}`,class:"gtl"}})};nhentai.errorTip=t=>helper.t("site.ehentai.nhentai_failed",{nhentai:`<a href='https://nhentai.net/search/?q=${t.galleryTitle}' target="_blank"> <u> nhentai </u> </a>`});const hitomi=async({setState,galleryId})=>{const domain="gold-usergeneratedcontent.net",downImg=async t=>{const o=await main.request(t,{headers:{Referer:`https://hitomi.la/reader/${galleryId}.html`},responseType:"blob",fetch:!1});return URL.createObjectURL(o.response)},res=await main.request(`https://ltn.${domain}/galleries/${galleryId}.js`,{errorText:helper.t("site.ehentai.hitomi_error"),noTip:!0}),data=JSON.parse(res.responseText.slice(18)),itemId=`@hitomi:${data.id}`;return setState("comicMap",itemId,{getImgList:({dynamicLoad})=>dynamicLoad(async setImg=>{const{responseText:ggScript}=await main.request(`https://ltn.${domain}/gg.js?_=${Date.now()}`,{errorText:helper.t("site.ehentai.hitomi_error"),noTip:!0});let gg={};eval(ggScript);const imgList=data.files.map(({hash:t})=>{const o=gg.s(t),a=/[\da-f]{61}([\da-f]{2})([\da-f])/.exec(t),n=Number.parseInt(a[2]+a[1],16);return`https://w${gg.m(n)+1}.${domain}/${gg.b}${o}/${t}.webp`});for(const[t,o]of imgList.entries())setImg(t,await downImg(o))},data.files.length,itemId)}),[{id:itemId,showText:data.id,title:data.title,href:`https://hitomi.la/galleries/${data.id}`,class:"gt"}]};hitomi.errorTip=()=>helper.t("site.ehentai.hitomi_error");const crossSiteLink=async t=>{if(!helper.querySelector("#gdc > .cs:is(.ct2, .ct3)"))return;if(!t.galleryTitle)return main.toast.error(helper.t("site.ehentai.html_changed_link_failed"));const[o,a]=store.createStore({}),n=l=>(()=>{var m=web.template("<div><a>")(),f=m.firstChild;return web.setStyleProperty(m,"opacity","1.0"),web.effect(h=>{var b=`td_${l.id}`,u=l.class,w=l.title,S=l.id,x=l.href,$=`return toggle_tagmenu(1, '${l.id}',this)`,v=l.title,y=l.showText;return b!==h.e&&web.setAttribute(m,"id",h.e=b),u!==h.t&&web.className(m,h.t=u),w!==h.a&&web.setAttribute(m,"title",h.a=w),S!==h.o&&web.setAttribute(f,"id",h.o=S),x!==h.i&&web.setAttribute(f,"href",h.i=x),$!==h.n&&web.setAttribute(f,"onclick",h.n=$),v!==h.s&&web.setAttribute(f,"title",h.s=v),y!==h.h&&(f.innerText=h.h=y),h},{e:void 0,t:void 0,a:void 0,o:void 0,i:void 0,n:void 0,s:void 0,h:void 0}),m})(),r=()=>web.render(()=>web.createComponent(solidJs.For,{get each(){return Object.entries(o)},children:([l,m])=>(()=>{var f=web.template("<tr><td class=tc>:")(),h=f.firstChild,b=h.firstChild;return web.setAttribute(f,"id",`${l}_tagline`),web.insert(h,l,b),web.insert(f,web.createComponent(solidJs.Show,{when:typeof m!="string",get fallback(){return(()=>{var u=web.template("<td class=tc style=text-align:left>")();return u.innerHTML=m,u})()},get children(){var u=web.template("<td>")();return web.insert(u,web.createComponent(solidJs.For,{each:m,children:n})),u}}),null),f})()}),helper.querySelector("#taglist tbody"));r(),helper.hijackFn("tag_update_vote",()=>{for(const l of helper.querySelectorAll("#nh_tagline"))l.remove();r()});const s=()=>web.template('<img src=https://ehgt.org/g/mr.gif class=mr alt=">">')(),_=l=>web.createComponent(solidJs.For,{get each(){return l.children},children:m=>[s(),m]}),p=document.getElementById("tagmenu_act");let c;helper.hijackFn("_refresh_tagmenu_act",(l,[m])=>{if(c?.(),!m.id.startsWith("@"))return l(m);p.children.length>0&&(p.innerHTML=""),c=web.render(()=>web.createComponent(_,{get children(){return[(()=>{var f=web.template("<a target=_blank>")();return f.innerText=" Jump",web.effect(()=>web.setAttribute(f,"href",m.href)),f})(),web.createComponent(t.LoadButton,{get id(){return m.id}})]}}),p)});for(const l of[hitomi,nhentai]){a(l.name,"searching...");try{const m=await l(t);m.length>0?a(l.name,m):a(l.name,"null")}catch(m){const f=l.errorTip(t);console.error(f,m),a(l.name,f)}}const{adList:d}=t.store.comicMap[""];if(d)for(const l of Object.values(o))typeof l!="string"&&l.length===1&&t.setState("comicMap",l[0].id,{adList:d})},detectAd=({store:{comicMap:t},setState:o,options:a,imgList:n,pageList:r,fileNameList:s})=>{if(!(a.detect_ad&&document.getElementById("ta_other:extraneous_ads")))return;o("comicMap","","adList",new main.ReactiveSet);const p=[];for(const c of helper.querySelectorAll("#gdt > a")){const d=Number(/.+-(\d+)/.exec(c.href)?.[1])-1;if(Number.isNaN(d))continue;r[d]=c.href;const l=c.querySelector("[title]");[,s[d]]=l.title.split(/：|: /),p[d]=l.tagName==="IMG"?l:/url\("(.+)"\)/.exec(l.style.backgroundImage)[1]}return(async()=>(await detectAd$1.getAdPageByFileName(s,t[""].adList),t[""].adList.size===0&&await detectAd$1.getAdPageByContent(p,t[""].adList),helper.useStyle(helper.createRootMemo(()=>t[""]?.adList?.size?[...t[""].adList].map(c=>`a[href="${r[c]}"] [title]:not(:hover) {
              filter: blur(8px);
...
`):""))))(),()=>{detectAd$1.getAdPageByFileName(s,t[""].adList),detectAd$1.getAdPageByContent(n,t[""].adList)}},expandTagList=t=>{if(t.type!=="t")return;helper.useStyle(`
  ...
  `);const o=new Map,a=async s=>{if(s.style.cursor!=="progress"){if(!o.has(s)){let _,p=null;try{s.dataset.tagListLoading="";const c=await main.request(s.querySelector("a").href,{noTip:!0,errorText:"Fetch tag list error",noCheckCode:!0});if(_=helper.domParse(c.responseText),p=_.querySelector("#taglist"),!p)throw new Error("Fetch tag list error");const[,d]=_.querySelector("#gdt div[title][style]").style.background.split('"');new Image().src=d;for(const l of p.querySelectorAll("a"))l.target="_blank"}catch{p=document.createElement("div"),p.id="taglist",p.textContent=_?.querySelector(".d p")?.textContent||"Fetch tag list error"}s.querySelector(".gl3t").after(p),o.set(s,p),Reflect.deleteProperty(s.dataset,"tagListLoading")}Reflect.has(s.dataset,"showTagList")?Reflect.deleteProperty(s.dataset,"showTagList"):s.dataset.showTagList=""}};for(const s of helper.querySelectorAll(".gl1t"))s.addEventListener("click",_=>_.target.matches(":not(a):is(.gl1t, .gl6t, .gl6t *, #taglist, #taglist *)")&&a(s));Manga.setDefaultHotkeys(s=>({...s,float_tag_list:["q"]}));const[n,r]=solidJs.createSignal([0,0]);document.addEventListener("pointermove",s=>r([s.clientX,s.clientY])),helper.linstenKeydown(s=>{const _=helper.getKeyboardCode(s);if(Manga.hotkeysMap()[_]!=="float_tag_list")return;s.stopPropagation(),s.preventDefault();const p=document.elementsFromPoint(...n());for(const c of p)if(c.matches(".gl1t"))return a(c)},!0),colorizeTag("gallery")},defaultOptions={cross_site_link:!0,add_hotkeys_actions:!0,detect_ad:!0,quick_favorite:!0,colorize_tag:!1,quick_rating:!0,quick_tag_define:!0,float_tag_list:!1,auto_adjust_option:!1,tag_lint:!1,expand_tag_list:!0,autoShow:!1},createEhContext=async()=>{let t;if(Reflect.has(unsafeWindow,"display_comment_field")?t="gallery":location.pathname==="/mytags"?t="mytags":Reflect.has(unsafeWindow,"mpvkey")?t="mpv":t=helper.querySelector('option[value="t"]')?.parentElement?.value,!t)return;const o=await main.useInit("ehentai",defaultOptions);if(t!=="gallery")return{type:t,...o};let a=0;if(a=Number(helper.querySelector(".gtb .gpc")?.textContent?.replaceAll(",","").match(/\d+/g)?.at(-1)),Number.isNaN(a)){const{responseText:n}=await main.request(location.href);a=Number(/(?<=class="gdt2">)\d+(?= pages<\/td>)/.exec(n)?.[0])}return{type:"gallery",...o,galleryId:Number(location.pathname.split("/")[2]),galleryTitle:helper.querySelector("#gn")?.textContent||void 0,japanTitle:helper.querySelector("#gj")?.textContent||void 0,imgNum:a,imgList:[],pageList:[],fileNameList:[],LoadButton(n){const r=solidJs.createMemo(()=>{const s=o.store.comicMap[n.id]?.imgList,_=s?.filter(Boolean).length;switch(s?.length){case void 0:return" Load comic";case _:return" Read";default:return` loading - ${_}/${s.length}`}});return(()=>{var s=web.template("<a href=javascript:;>")();return s.$$click=async _=>{await n.onClick?.(_),o.showComic(n.id)},web.insert(s,r),s})()},dom:{newTagField:document.getElementById("newtagfield")}}};web.delegateEvents(["click"]);const escHandler=[],setEscHandler=(t,o)=>{escHandler.push(Object.assign(o,{order:t})),escHandler.sort((a,n)=>n.order-a.order)},getTaglist=()=>{const t=new Set,o=new Set;for(const a of helper.querySelectorAll("#taglist table [id^=td_]")){const[n]=a.getElementsByTagName("a");n.classList.contains("tdn")||(n.classList.contains("tup")||a.classList.contains("gt")?t.add(a.id.slice(3)):a.classList.contains("gtl")&&o.add(a.id.slice(3)))}return[t,o]},handleTagName=t=>{const[o,a]=t.trim().split(":");return a?[o,a.replaceAll(/[^a-z-_ ]/gi,"")]:["",""]},namespaceAbbr=[["artist","a"],["character","c","char"],["cosplayer","c","os"],["female","f"],["group","g","circle"],["language","l","lang"],["male","m"],["mixed","x"],["other","o"],["parody","p","series"],["reclass","r"]],getTagNameFull=t=>{const[o,a]=handleTagName(t);for(const n of namespaceAbbr)if(n.includes(o))return`${n[0]}:${a}`;return t},MdPictureInPicture='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke="currentColor" fill="currentColor" stroke-width="0"><path d="M18 7h-6c-.55 0-1 .45-1 1v4c0 .55.45 1 1 1h6c.55 0 1-.45 1-1V8c0-.55-.45-1-1-1m3-4H3c-1.1 0-2 .9-2 2v14c0 1.1.9 1.98 2 1.98h18c1.1 0 2-.88 2-1.98V5c0-1.1-.9-2-2-2m-1 16.01H4c-.55 0-1-.45-1-1V5.98c0-.55.45-1 1-1h16c.55 0 1 .45 1 1v12.03c0 .55-.45 1-1 1"/></svg>',getDomPosition=t=>{const o=t.getBoundingClientRect(),a=getComputedStyle(t),n=Number.parseFloat(a.borderLeftWidth),r=Number.parseFloat(a.paddingLeft),s=Number.parseFloat(a.paddingTop),_=Number.parseFloat(a.borderTopWidth);return{left:o.left+n+r,top:o.top+_+s,width:a.width,height:a.height}},floatTagList=({store:{manga:t},dom:{newTagField:o}})=>{const a=helper.querySelector("#gd4"),n=getComputedStyle(a);let r="rgba(0, 0, 0, 0)",s=a;for(;r==="rgba(0, 0, 0, 0)";)r=getComputedStyle(s).backgroundColor,s=s.parentElement;const{borderColor:_}=getComputedStyle(helper.querySelector("#gdt")),p=`1px solid ${_}`;helper.useStyle(`
      #comicread-tag-box {
        position: fixed;
        z-index: 2147483647;
    ...
    `);const{store:c,setState:d}=helper.useStore({open:!1,top:0,left:0,opacity:1,mouse:{x:0,y:0},bound:{width:0,height:0}}),l=(g,k,I)=>{g.top=helper.clamp(-a.clientHeight*.75,k,g.bound.height),g.left=helper.clamp(-a.clientWidth*.75,I,g.bound.width)},m=g=>d("opacity",helper.clamp(.5,g,1));m(Number(localStorage.getItem("floatTagListOpacity"))||1),document.addEventListener("pointermove",g=>{d(k=>{k.mouse.x=g.clientX,k.mouse.y=g.clientY})});const f=()=>{d(g=>{g.bound.width=window.innerWidth-a.clientWidth/4,g.bound.height=window.innerHeight-a.clientHeight/4,l(g,g.top,g.left)})};window.addEventListener("resize",f),f(),helper.useStyleMemo("#comicread-tag-box",{display:()=>c.open?void 0:"none",top:()=>`${c.top}px`,left:()=>`${c.left}px`,opacity:()=>c.opacity});const h=a.cloneNode();h.id="comicread-tag-box-placeholder",h.style.display="none",h.addEventListener("click",()=>d("open",!1)),h.innerHTML=MdPictureInPicture,a.parentElement.append(h);const b=document.createElement("div");b.id="comicread-tag-box",b.classList.add("comicread-ignore"),document.body.append(b),b.addEventListener("wheel",g=>{g.shiftKey&&(g.stopPropagation(),g.preventDefault(),m(c.opacity+(g.deltaY>0?-.05:.05)),localStorage.setItem("floatTagListOpacity",`${c.opacity}`))},{passive:!1});const u={top:0,left:0};helper.useDrag({ref:a,handleDrag({type:g,xy:[k,I],initial:[L,C]}){switch(g){case"down":if(!c.open){const M=getDomPosition(a);d(T=>{T.top=M.top,T.left=M.left})}u.top=c.top,u.left=c.left;break;case"up":d(M=>{if(t.show)return;const T=h.getBoundingClientRect();helper.approx(M.top,T.top,50)&&helper.approx(M.left,T.left,50)&&(M.open=!1)});break;case"move":d(M=>{l(M,u.top+I-C,u.left+k-L),M.open=!0});break}},handleClick:(g,k)=>k.click(),skip:g=>!g.target.matches("#gd4, #taglist, #gwrd, td+td, [id^=comidread] *:not(a)")});let w,S;const x=()=>{if(w||(w=helper.querySelector("#ehs-introduce-box"),!w))return;S=w.parentElement;const g=helper.querySelector(".eh-syringe-lite-auto-complete-list");g&&(g.classList.add("comicread-ignore"),g.style.zIndex="2147483647",document.body.append(g)),helper.hijackFn("toggle_tagmenu",()=>unsafeWindow.selected_tagname||helper.querySelector("#ehs-introduce-box .ehs-close")?.click())};helper.createEffectOn(()=>c.open,g=>{if(x(),g){const{height:k,width:I}=n;h.style.cssText=`height: ${k}; width: ${I};`,b.style.height=k,a.style.width=I,b.append(a),w&&b.append(w),document.activeElement.blur()}else h.style.cssText="display: none;",a.style.width="",h.after(a),w&&S.append(w),Manga.focus()},{defer:!0}),Manga.setDefaultHotkeys(g=>({...g,float_tag_list:["q"]})),setEscHandler(0,()=>c.open?d("open",!1):!0),helper.linstenKeydown(g=>{const k=helper.getKeyboardCode(g);Manga.hotkeysMap()[k]==="float_tag_list"&&(g.stopPropagation(),g.preventDefault(),d(I=>{I.open=!I.open,I.open&&l(I,I.mouse.y-a.clientHeight/2,I.mouse.x-a.clientWidth/2)}))}),helper.hijackFn("tag_from_field",(g,k)=>(c.open&&document.activeElement.blur(),g(...k))),o.addEventListener("pointerenter",()=>c.open&&o.focus());const $=g=>{const k=helper.querySelector(`a[href=${CSS.escape(g)}]`);if(k)return k.title||k.id.slice(3).replaceAll("_"," ")},v=g=>{const k=g.dataTransfer.getData("text"),I=$(k);I&&(g.preventDefault(),o.value.includes(I)||(o.value+=`${I}, `),o.dispatchEvent(new Event("input")))};o.addEventListener("drop",v);const y=helper.querySelector("#taglist");y.addEventListener("dragover",g=>g.preventDefault()),y.addEventListener("dragenter",g=>g.preventDefault()),y.addEventListener("drop",v)},addHotkeysActions=t=>{t.options.add_hotkeys_actions&&(t.type==="gallery"?(setEscHandler(0,()=>unsafeWindow.selected_tagname?unsafeWindow.toggle_tagmenu():!0),helper.linstenKeydown(o=>{if(unsafeWindow.selected_tagid)switch(o.key){case"ArrowUp":return o.preventDefault(),unsafeWindow?.tag_vote_up();case"ArrowDown":return o.preventDefault(),unsafeWindow?.tag_vote_down()}switch(Manga.hotkeysMap()[helper.getKeyboardCode(o)]){case"scroll_right":return o.preventDefault(),helper.querySelector(".ptt td:last-child:not(.ptdd)")?.click();case"scroll_left":return o.preventDefault(),helper.querySelector(".ptt td:first-child:not(.ptdd)")?.click()}})):helper.linstenKeydown(o=>{switch(Manga.hotkeysMap()[helper.getKeyboardCode(o)]){case"scroll_right":return o.preventDefault(),helper.querySelector("#unext")?.click();case"scroll_left":return o.preventDefault(),helper.querySelector("#uprev")?.click()}}))},style=`
  .comidread-favorites {
    position: absolute;
    z-index: 75;
    left: 0;
...
`,addQuickFavorite=(favoriteButton,root,apiUrl,height,top=0)=>{root.style.position="relative";const[show,setShow]=solidJs.createSignal(!1),[favorites,setFavorites]=solidJs.createSignal([]),[favnote,setFavnote]=solidJs.createSignal(""),updateFavorite=async()=>{try{const t=await main.request(apiUrl,{errorText:helper.t("site.ehentai.fetch_favorite_failed")}),o=helper.domParse(t.responseText),a=[...o.querySelectorAll(".nosel > div")];a.length===10&&(a[0].querySelector("input").checked=!1),setFavnote(o.querySelector('#galpop textarea[name="favnote"]')?.value??""),setFavorites(a)}catch{main.toast.error(helper.t("site.ehentai.fetch_favorite_failed")),setFavorites([])}};let hasRender=!1;const renderDom=()=>{if(hasRender)return;hasRender=!0;const FavoriteItem=(e,index)=>{const{checked}=e.querySelector("input"),handleClick=async()=>{if(checked)return;setShow(!1);const formData=new FormData;formData.append("favcat",index()===10?"favdel":`${index()}`),formData.append("apply","Apply Changes"),formData.append("favnote",favnote()),formData.append("update","1");const res=await main.request(apiUrl,{method:"POST",data:formData,errorText:helper.t("site.ehentai.change_favorite_failed")});main.toast.success(helper.t("site.ehentai.change_favorite_success"));const updateCode=/\nif\(window.opener.document.+\n/.exec(res.responseText)?.[0]?.replaceAll("window.opener.document","window.document");updateCode&&eval(updateCode),await updateFavorite()};return(()=>{var t=web.template("<div class=comidread-favorites-item><input type=radio>")(),o=t.firstChild;return t.$$click=handleClick,o.checked=checked,web.insert(t,web.createComponent(solidJs.Show,{get when(){return index()<=9},get children(){var a=web.template("<div>")();return web.effect(n=>web.setStyleProperty(a,"background-position",`0px -${2+19*index()}px`)),a}}),null),web.insert(t,()=>e.textContent?.trim(),null),t})()};let background="rgba(0, 0, 0, 0)",dom=root;for(;background==="rgba(0, 0, 0, 0)";)background=getComputedStyle(dom).backgroundColor,dom=dom.parentElement;web.render(()=>web.createComponent(solidJs.Show,{get when(){return show()},get children(){var t=web.template("<span class=comidread-favorites>")();return web.setStyleProperty(t,"background",background),web.setStyleProperty(t,"height",`${height}px`),web.setStyleProperty(t,"top",`${top}px`),web.insert(t,web.createComponent(solidJs.For,{get each(){return favorites()},children:FavoriteItem,get fallback(){return web.template("<h3>loading...")()}})),t}}),root)},rawClick=favoriteButton.onclick;favoriteButton.onclick=null,favoriteButton.addEventListener("mousedown",async t=>{if(!(t.buttons!==1&&t.buttons!==4)){if(t.stopPropagation(),t.preventDefault(),t.shiftKey||t.ctrlKey||t.altKey||t.metaKey||t.buttons===4)return rawClick.call(favoriteButton,t);renderDom(),setShow(o=>!o),show()&&await updateFavorite()}})},quickFavorite=t=>{switch(t.type){case"gallery":{helper.useStyle(style);const o=helper.querySelector("#gdf"),a=helper.querySelector("#gd3"),n=o.firstElementChild.offsetTop;addQuickFavorite(o,a,`${unsafeWindow.popbase}addfav`,n);break}case"t":{helper.useStyle(style);for(const o of helper.querySelectorAll(".gl1t")){const a=o.querySelector("[id^=posted_]"),n=o.firstElementChild.getBoundingClientRect().bottom-o.getBoundingClientRect().top,r=o.lastElementChild.getBoundingClientRect().top-o.getBoundingClientRect().top,[s]=/http.+?(?=')/.exec(a.getAttribute("onclick"));addQuickFavorite(a,o,s,r-n,n)}break}case"e":{helper.useStyle(style);for(const o of helper.querySelectorAll(".gl1e")){const a=o.nextElementSibling.querySelector("[id^=posted_]"),n=Number.parseInt(getComputedStyle(o).height,10),[r]=/http.+?(?=')/.exec(a.getAttribute("onclick"));addQuickFavorite(a,o,r,n)}break}}};web.delegateEvents(["click"]);const quickRating=t=>{let o;switch(t.type){case"e":o=helper.querySelectorAll("#favform > table > tbody > tr");break;case"m":case"p":case"l":o=helper.querySelectorAll("#favform > table > tbody > tr").slice(1);break;case"t":o=helper.querySelectorAll(".gl1t");break;default:return}helper.useStyle(`
  ...
  `);const a=["0,0,7,16","8,0,15,16","16,0,23,16","24,0,31,16","32,0,39,16","40,0,47,16","48,0,55,16","56,0,63,16","64,0,71,16","72,0,79,16"],n=async(_,p)=>{try{const c=await main.request(_,{errorText:helper.t("site.ehentai.change_rating_failed"),noTip:!0}),d=/api_url = "(.+?)";.+?gid = (\d+);.+?token = "(.+?)";.+?apiuid = (\d+);.+?apikey = "(.+?)"/s.exec(c.responseText);if(!d)throw new Error(helper.t("site.ehentai.change_rating_failed"));const[,l,m,f,h,b]=d,u=await main.request(l,{method:"POST",responseType:"json",data:JSON.stringify({method:"rategallery",rating:`${p}`,apikey:b,apiuid:h,gid:m,token:f}),fetch:!0,noTip:!0});return main.toast.success(`${helper.t("site.ehentai.change_rating_success")}: ${u.response.rating_usr}`),u.response}catch{throw main.toast.error(helper.t("site.ehentai.change_rating_failed")),new Error(helper.t("site.ehentai.change_rating_failed"))}},r=(_,p)=>{let c=Math.round(p+1);const d=-80+16*Math.ceil(c/2);c=c%2===1?-21:-1,_.style.backgroundPosition=`${d}px ${c}px`},s=(_,p,c)=>{let d=p.style.backgroundPosition;web.render(()=>(()=>{var l=web.template("<span class=comidread-quick-rating><img src=https://ehgt.org/g/blank.gif><map>")(),m=l.firstChild,f=m.nextSibling;return l.$$mouseout=()=>{p.style.backgroundPosition=d},web.setAttribute(l,"data-index",c),web.setAttribute(m,"usemap",`#rating-${c}`),web.setAttribute(f,"name",`rating-${c}`),web.insert(f,web.createComponent(solidJs.For,{each:a,children:(h,b)=>(()=>{var u=web.template("<area shape=rect>")();return u.$$click=async()=>{const w=await n(_.querySelector("a").href,b()+1);p.className=w.rating_cls,r(p,w.rating_usr*2-1),d=p.style.backgroundPosition},u.$$mouseover=()=>r(p,b()),web.setAttribute(u,"coords",h),u})()})),l})(),p)};for(const[_,p]of o.entries()){const c=[...p.querySelectorAll(".ir")].at(-1);c&&c.addEventListener("mouseenter",()=>s(p,c,_),{once:!0})}};web.delegateEvents(["mouseout","mouseover","click"]);const MDLaunch=(t={})=>(()=>{var o=web.template('<svg xmlns=http://www.w3.org/2000/svg viewBox="0 0 24 24"stroke=currentColor fill=currentColor stroke-width=0><path d="M18 19H6c-.55 0-1-.45-1-1V6c0-.55.45-1 1-1h5c.55 0 1-.45 1-1s-.45-1-1-1H5a2 2 0 0 0-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2v-6c0-.55-.45-1-1-1s-1 .45-1 1v5c0 .55-.45 1-1 1M14 4c0 .55.45 1 1 1h2.59l-9.13 9.13a.996.996 0 1 0 1.41 1.41L19 6.41V9c0 .55.45 1 1 1s1-.45 1-1V3h-6c-.55 0-1 .45-1 1">')();return web.spread(o,t,!0,!0),o})(),quickTagDefine=t=>{const o=store.createMutable({}),a=async c=>{if(Reflect.has(o,c))return;const d=`https://ehwiki.org/wiki/${c.replaceAll(/[a-z]+:\s?/gi,"")}`,l=await main.request(d,{noCheckCode:!0});if(l.status!==200){o[c]=(()=>{var h=web.template("<h3>")();return web.insert(h,()=>`${l.status} - ${l.statusText}`),h})();return}const f=helper.domParse(l.responseText).querySelector("#mw-content-text");for(const h of f.querySelectorAll('img[src^="/"]'))h.setAttribute("src",`https://ehwiki.org${h.getAttribute("src")}`);for(const h of f.getElementsByTagName("a")){const b=h.getAttribute("href")??"";b.startsWith("/")&&h.setAttribute("href",`https://ehwiki.org${b}`),h.target="_blank"}for(const h of f.querySelectorAll(".thumb"))h.remove();o[c]=[(()=>{var h=web.template("<h1><a target=_blank>")(),b=h.firstChild;return web.setAttribute(b,"href",d),web.insert(b,c,null),web.insert(b,web.createComponent(MDLaunch,{}),null),h})(),f]};helper.useStyle(`
  ...
  `);const[n,r]=solidJs.createSignal(!1),s=helper.querySelector("#taglist");let _="rgba(0, 0, 0, 0)",p=s;for(;_==="rgba(0, 0, 0, 0)";)_=getComputedStyle(p).backgroundColor,p=p.parentElement;web.render(()=>web.createComponent(solidJs.Show,{get when(){return n()},get children(){var c=web.template("<span id=comidread-tag-define>")();return web.setStyleProperty(c,"background",_),web.insert(c,()=>o[unsafeWindow.selected_tagname]??web.template("<h3>loading...")()),web.effect(d=>web.setStyleProperty(c,"height",`${s.scrollHeight}px`)),c}}),s),unsafeWindow.tag_define=async()=>{if(unsafeWindow.selected_tagname){if(n())return r(!1);r(!0);try{await a(unsafeWindow.selected_tagname)}catch(c){console.error(c),r(!1)}}},helper.hijackFn("toggle_tagmenu",()=>r(!1)),setEscHandler(2,()=>n()?r(!1):!0)},updateSortCss=t=>{let o="tr a :is(.gltm, .glink + div:not([class])) { display: flex; }";for(const{title:a,order:n}of t)o+=`
.gt[title="${a}"] { order: ${n}; }`;return GM.setValue("ehTagSortCss",o)},sortTags=async t=>{switch(handleMyTagsChange.add(updateSortCss),t.type){case"p":case"l":case"t":return helper.useStyle(await helper.getGmValue("ehTagSortCss",updateMyTags));case"mytags":{let o;const a=n=>{let r=`
          ...
          #usertag_0 { order: -${n.length}; }`;for(const{order:s,id:_}of n)r+=`
#usertag_${_} { view-transition-name: _${_}; order: ${s}; }`;o||=GM_addElement("style",{textContent:r}),o.textContent=r};handleMyTagsChange.add(n=>{if(!document.startViewTransition)return a(n);document.startViewTransition(()=>a(n))})}}},tagLint=({dom:{newTagField:t}})=>{const o=helper.querySelector("#gdc > .cs:is(.ct2, .ct3, .ct9)"),a=ehTagRules.getTagLintRules(),[n,r]=solidJs.createSignal({});helper.useStyle(`
    #comidread-tag-lint [id^=td_] {
      display: inline-block;
      float: none;
    }
  `);const s=(u,w)=>w===void 0?document.getElementById(`td_${u}`)?.className:w?"gtl":"gt",_=u=>(()=>{var w=web.template("<div><a>")(),S=w.firstChild;return S.$$click=x=>x.preventDefault(),web.insert(S,()=>u.name),web.effect(x=>{var $=`td_${u.name}`,v=s(u.name,u.weak),y=`ta_${u.name}`,g=`https://exhentai.org/tag/${u.name.replaceAll("_","+")}`;return $!==x.e&&web.setAttribute(w,"id",x.e=$),v!==x.t&&web.className(w,x.t=v),y!==x.a&&web.setAttribute(S,"id",x.a=y),g!==x.o&&web.setAttribute(S,"href",x.o=g),x},{e:void 0,t:void 0,a:void 0,o:void 0}),w})(),p=u=>{const w=ehTagRules.splitTagNamespace(u.name);return web.createComponent(solidJs.Show,{get when(){return w.length>1},get fallback(){return _(u)},get children(){var S=web.template("<span>\u300C<!> \u300D")(),x=S.firstChild,$=x.nextSibling;return web.insert(S,web.createComponent(solidJs.For,{each:w,children:(v,y)=>[web.memo(()=>web.memo(()=>!!y())()?` ${helper.t("other.or")} `:""),web.createComponent(_,{name:v,get weak(){return u.weak}})]}),$),S}})},c=u=>{const[w,S,x]=u.text.split("[tag]");return web.createComponent(solidJs.Show,{get when(){return u.warnList?.size},get children(){return web.createComponent(solidJs.For,{get each(){return[...u.warnList.entries()]},children:([$,v])=>(()=>{var y=web.template("<li>")();return web.insert(y,w,null),web.insert(y,web.createComponent(p,{name:$}),null),web.insert(y,S,null),web.insert(y,web.createComponent(solidJs.For,{each:v,children:g=>web.createComponent(p,{name:g,get weak(){return u.weak}})}),null),web.insert(y,x,null),y})()})}})};let d,l;const m=helper.singleThreaded(()=>{const u={},[w,S]=getTaglist(),x=new Set([...w,...S]),$=(g,k,I=!1)=>{const L=a[k];if(L.has(g))for(const C of L.get(g)){if(ehTagRules.hasTag(I?w:x,C)===I)continue;u[k]??=new Map([[g,[]]]);const M=u[k];M.has(g)||M.set(g,[]),M.get(g).push(C)}};for(const g of x)$(g,"prerequisite",!0),$(g,"conflict"),o&&$(g,"possibleConflict"),$(g,"combo",!0);const v=(g,k)=>{u.other??=[],u.other.push([g,k])},y=[];for(const g of S)if(/^(?:artist|group):/.test(g)){const k=helper.querySelector("#gd2").textContent.toLowerCase();if(k.includes(g.replaceAll(/^(artist|group):|_/g," ").trim()))y.push(g);else{const I=document.getElementById(`ta_${g}`)?.textContent;I&&k.includes(I)&&y.push(g)}}y.length>0&&v(helper.t("eh_tag_lint.correct_tag"),y),helper.querySelector("#gdc > .cs.ct2")&&ehTagRules.isMissingNamespace(x,"parody")&&v(helper.t("eh_tag_lint.miss_parody"),["parody:original"]),o&&ehTagRules.isMissingTags(w,"female:females_only","female:futanari","female:shemale")&&ehTagRules.isMissingNamespace(x,"male","mixed")&&v(helper.t("eh_tag_lint.miss_female"),["female:females_only"]),r(u),d?.isConnected||(d=document.createElement("div"),d.id="comidread-tag-lint",helper.querySelector("#taglist").append(d)),l?.(),l=web.render(()=>web.createComponent(solidJs.Show,{get when(){return Object.keys(n()).length},get children(){return[web.template("<hr>")(),(()=>{var g=web.template("<ul>")();return web.insert(g,web.createComponent(solidJs.For,{get each(){return n().other},children:([k,I])=>(()=>{var L=web.template("<li>")();return web.insert(L,k,null),web.insert(L,web.createComponent(solidJs.For,{each:I,children:C=>web.createComponent(p,{name:C,weak:!0})}),null),L})()}),null),web.insert(g,web.createComponent(c,{get warnList(){return n().prerequisite},get text(){return helper.t("eh_tag_lint.prerequisite")},weak:!1}),null),web.insert(g,web.createComponent(c,{get warnList(){return n().conflict},get text(){return helper.t("eh_tag_lint.conflict")}}),null),web.insert(g,web.createComponent(c,{get warnList(){return n().possibleConflict},get text(){return helper.t("eh_tag_lint.possible_conflict")}}),null),web.insert(g,web.createComponent(c,{get warnList(){return n().combo},get text(){return helper.t("eh_tag_lint.combo")},weak:!0}),null),g})()]}}),d)});m(),helper.hijackFn("tag_update_vote",m);const[f,h]=helper.createEqualsSignal([]);helper.useStyle(helper.createRootMemo(()=>f().map(u=>`#td_${CSS.escape(u.replaceAll(" ","_"))} { box-shadow: 0px 0px 4px var(--tag); }`).join(`
`)));const b=()=>h(t.value.split(",").map(u=>getTagNameFull(u.trim())).filter(Boolean));t.addEventListener("input",b),t.addEventListener("keydown",b),helper.hijackFn("tag_update_vote",b)};web.delegateEvents(["click"]),(async()=>{const t=await createEhContext();if(!t)return;const{setState:o,options:a,setOptions:n,showComic:r}=t,s=()=>[web.createComponent(solidJs.For,{each:["colorize_tag","float_tag_list","expand_tag_list","tag_lint","","quick_favorite","quick_rating","quick_tag_define","","cross_site_link","detect_ad","add_hotkeys_actions","auto_adjust_option"],children:v=>web.createComponent(solidJs.Show,{when:v,get fallback(){return web.template("<hr>")()},get children(){return web.createComponent(Manga.SettingsItemSwitch,{get name(){return helper.t(`site.add_feature.${v}`)},get value(){return a[v]},onChange:y=>n({[v]:y})})}})}),web.template("<hr>")(),web.createComponent(Manga.SettingBlockSubtitle,{get children(){return helper.t("other.hotkeys")}}),web.createComponent(Manga.SettingHotkeys,{keys:["float_tag_list"]})];if(o(v=>{v.manga.editSettingList=y=>[...y,["E-Hentai",s]],v.fab.otherSpeedDial=["tag_lint","colorize_tag","cross_site_link","detect_ad"]}),t.type==="mpv")return o("comicMap","",{getImgList({dynamicLoad:v}){const y=helper.querySelectorAll(".mimg[id]");return v(k=>{const I=unsafeWindow.imagelist;helper.plimit(I.map((L,C)=>async()=>{const M=()=>I[C].i;for(;!M();)Reflect.has(I[C],"xhr")||(unsafeWindow.load_image(C+1),unsafeWindow.next_possible_request=0),await helper.wait(M);k(C,M())}),void 0,4)},y.length)}});if(helper.linstenKeydown(v=>{if(v.key==="Escape"){for(const y of escHandler)if(y()!==!0)return v.stopImmediatePropagation()}}),a.colorize_tag&&(colorizeTag(t.type),sortTags(t)),unsafeWindow.apiuid!==-1&&a.quick_favorite&&helper.requestIdleCallback(()=>quickFavorite(t)),a.quick_rating&&helper.requestIdleCallback(()=>quickRating(t),1e3),a.expand_tag_list&&helper.requestIdleCallback(()=>expandTagList(t),1e3),t.type!=="gallery")return addHotkeysActions(t);if(a.auto_adjust_option&&!helper.querySelector("#gdc > .cs:is(.ct2, .ct3, .ct9)")){let v={pageNum:1,imgRecognition:{enabled:!1}};a.option&&(v=helper.assign(a.option,v)),o("manga","option",v)}a.float_tag_list&&helper.requestIdleCallback(()=>floatTagList(t)),a.quick_tag_define&&helper.requestIdleCallback(()=>quickTagDefine(),1e3),a.tag_lint&&helper.requestIdleCallback(()=>tagLint(t),1e3);const _=document.getElementById("gd5");if(new ResizeObserver(()=>{Reflect.deleteProperty(_.dataset,"long"),_.scrollHeight>352&&(_.dataset.long="")}).observe(_),helper.useStyle(`
    #gd5[data-long] {
  ...
  `),a.cross_site_link&&helper.requestIdleCallback(()=>crossSiteLink(t),1e3),Number.isNaN(t.imgNum))return main.toast.error(helper.t("site.changed_load_failed"));const c=await detectAd(t),d=v=>v.includes("IP address has been temporarily banned")&&main.toast.error(helper.t("site.ehentai.ip_banned"),{throw:!0,duration:Number.POSITIVE_INFINITY}),l=async v=>{const y=await main.request(v,{fetch:!0,errorText:helper.t("site.ehentai.fetch_img_page_source_failed")},10);d(y.responseText);try{return/id="img" src="(.+?)"/.exec(y.responseText)[1]}catch{throw new Error(helper.t("site.ehentai.fetch_img_url_failed"))}},m=async(v=0)=>{const y=await main.request(`${location.pathname}${v?`?p=${v}`:""}`,{fetch:!0,errorText:helper.t("site.ehentai.fetch_img_page_url_failed")});d(y.responseText);const g=[...y.responseText.matchAll(/<a href="(.{20,50})"><(img alt=.+?|div><div |div )title=".+?: (.+?)"/g)].map(([,k,,I])=>[k,I]);if(g.length===0)throw new Error(helper.t("site.ehentai.fetch_img_page_url_failed"));return g},[f,h]=solidJs.createSignal(`1-${t.imgNum}`),b=helper.createRootMemo(()=>helper.extractRange(f(),t.imgList.length||t.imgNum)),u=Number(helper.querySelector(".ptt td:nth-last-child(2)").textContent),w=async v=>{if(t.pageList.length!==u){const y=await helper.plimit(helper.range(u,g=>()=>m(g)));t.pageList.length=0,t.fileNameList.length=0;for(const g of y)for(const[k,I]of g)t.pageList.push(k),t.fileNameList.push(I)}await helper.plimit([...b()].map((y,g)=>async()=>{y<0||(t.imgList[y]||=await l(t.pageList[y]),v(g,t.imgList[y]))})),c?.()};o("comicMap","",{getImgList:({dynamicLoad:v})=>v(w,()=>b().size)});const S=await helper.useCache({pageRange:"id"});web.render(()=>{const v=_.children[6]?.classList.contains("gsp"),y=async g=>{if(!g.shiftKey)return;g.stopPropagation();const k=await S.get("pageRange",unsafeWindow.gid),I=prompt(helper.t("other.page_range"),k?.range);I&&(await S.set("pageRange",{id:unsafeWindow.gid??t.galleryId,range:I}),h(I??`1-${t.imgNum}`),o("comicMap","","imgList",void 0),r())};return(()=>{var g=web.template('<p class="g2 gsp"style=padding-bottom:0><img src=https://ehgt.org/g/mr.gif>')();return web.setStyleProperty(g,"padding-top",v?0:void 0),g.addEventListener("click",y,!0),web.insert(g,web.createComponent(t.LoadButton,{id:""}),null),g})()},_),addHotkeysActions(t);const x=async v=>{const y=await main.request(v,{errorText:helper.t("site.ehentai.fetch_img_page_source_failed")});d(y.responseText);const g=/nl\('(.+?)'\)/.exec(y.responseText)?.[1];if(!g)throw new Error(helper.t("site.ehentai.fetch_img_url_failed"));const k=new URL(v);return k.searchParams.set("nl",g),k.href},$=helper.singleThreaded(async(v,y)=>{const g=t.imgList.indexOf(y);if(g!==-1){if(t.imgList[g]=await l(t.pageList[g]),!await helper.testImgUrl(t.imgList[g])&&(t.pageList[g]=await x(t.pageList[g]),t.imgList[g]=await l(t.pageList[g]),main.toast.warn(helper.t("alert.retry_get_img_url",{i:g})),!await helper.testImgUrl(t.imgList[g])))return await helper.sleep(500),$(y);o("comicMap","","imgList",t.imgList);for(const k of Manga.imgList())if(k.loadType==="error")return $(k.src)}});o(v=>{v.manga.title=t.japanTitle||t.galleryTitle,v.manga.onExit=y=>{y&&helper.scrollIntoView("#cdiv"),o("manga","show",!1)},v.manga.onImgError=$,v.fab.initialShow=a.autoShow})})().catch(t=>helper.log.error(t));break}case"nhentai.net":{const t=require("solid-js/web"),o=require("helper"),a=require("main"),n=require("userscript/detectAd"),r={j:"jpg",p:"png",g:"gif",w:"webp",b:"bmp"};(async()=>{const{store:s,options:_,setState:p,showComic:c}=await a.useInit("nhentai",{auto_page_turn:!0,block_totally:!0,open_link_new_page:!0,detect_ad:!0});if(Reflect.has(unsafeWindow,"gallery")){p("manga",{onExit(h){h&&o.scrollIntoView("#comment-container"),p("manga","show",!1)}});const d=unsafeWindow._n_app.options.image_cdn_urls;p("comicMap","",{getImgList:()=>_gallery.images.pages.map((h,b)=>`https://${d[b%d.length]}/galleries/${_gallery.media_id}/${b+1}.${r[h.t]}`)}),p("fab","initialShow",_.autoShow);const m=(()=>{var h=t.template('<a href=javascript:; id=comicReadMode class="btn btn-secondary"><i class="fa fa-book"></i> Read')();return h.$$click=()=>c(),h})();document.getElementById("download").after(m),_.detect_ad&&o.querySelector("#tags .tag.tag-144644")&&(p("comicMap","","adList",new a.ReactiveSet),await n.getAdPageByContent(o.querySelectorAll(".thumb-container img").map(h=>h.dataset.src),s.comicMap[""].adList),o.createEffectOn(()=>s.comicMap[""].imgList,h=>h?.length&&n.getAdPageByContent(h,s.comicMap[""].adList)),o.useStyle(()=>s.comicMap[""]?.adList?.size?[...s.comicMap[""].adList].map(b=>`
            .thumb-container:nth-of-type(${b+1}):not(:hover) {
              filter: blur(8px);
              clip-path: border-box;
            }`).join(`
`):""));return}if(document.getElementsByClassName("gallery").length>0){if(_.open_link_new_page)for(const l of o.querySelectorAll('a:not([href^="javascript:"])'))l.setAttribute("target","_blank");const d=(unsafeWindow?._n_app??unsafeWindow?.n)?.options?.blacklisted_tags;if(d===void 0&&a.toast.error(o.t("site.nhentai.tag_blacklist_fetch_failed")),_.block_totally&&d?.length&&o.useStyle(".blacklisted.gallery { display: none; }"),_.auto_page_turn){let l=o.querySelector("a.next")?.href;if(!l)return;o.useStyle(`
      ...
      `);const m=new Set(d),f=document.getElementById("content"),h=()=>f.querySelector(":is(.index-container, #favcontainer):last-of-type"),b=o.singleThreaded(async()=>{if(!l)return;const w=await a.request(l,{fetch:!0,errorText:o.t("site.nhentai.fetch_next_page_failed")}),S=o.domParse(w.responseText);history.replaceState(null,"",l);const x=S.querySelector(".index-container, #favcontainer");for(const y of x.querySelectorAll(".gallery")){for(const k of y.getElementsByTagName("img"))k.setAttribute("src",k.dataset.src);y.dataset.tags.split(" ").map(Number).some(k=>m.has(k))&&y.classList.add("blacklisted")}const $=S.querySelector(".pagination");l=$.querySelector("a.next")?.href,f.append(x,$);const v=document.createElement("hr");f.append(v),u.disconnect(),u.observe(h()),l||(v.style.animationPlayState="paused")},{abandon:!0});b();const u=new IntersectionObserver(w=>w[0].isIntersecting&&b());u.observe(h()),o.querySelector("section.pagination")&&f.append(document.createElement("hr"))}}})().catch(s=>o.log.error(s)),t.delegateEvents(["click"]);break}case"yuri.website":{const t=require("helper"),o=require("main");(async()=>{const{store:a,setState:n,showComic:r,init:s}=await o.useInit("yurifans",{\u81EA\u52A8\u7B7E\u5230:!0});if(a.options.\u81EA\u52A8\u7B7E\u5230&&(async()=>{if(!globalThis.b2token)return;const p=new Date().toLocaleDateString("zh-CN");if(p!==localStorage.getItem("signDate"))try{const c=await o.request("/wp-json/b2/v1/userMission",{method:"POST",noTip:!0,headers:{Authorization:`Bearer ${b2token}`}}),d=JSON.parse(c.responseText);if(!(d?.mission?.date||!Number.isNaN(Number(d))))throw new Error("\u7B7E\u5230\u5931\u8D25");o.toast("\u81EA\u52A8\u7B7E\u5230\u6210\u529F"),localStorage.setItem("signDate",p)}catch{o.toast.error("\u81EA\u52A8\u7B7E\u5230\u5931\u8D25")}})(),!await t.waitDom('a.post-list-cat-item[title="\u5728\u7EBF\u533A-\u6F2B\u753B"]'))return;if(t.querySelector(".content-hidden")){const c=t.querySelector(".content-hidden").getElementsByTagName("img");await t.wait(()=>c.length,1e3)&&n("comicMap","",{getImgList:()=>[...c].map(l=>l.src)});return}if(t.querySelector(".xControl")){n("flag","needAutoShow",!1);const p=c=>{r(c),n("manga",{onPrev:Reflect.has(a.comicMap,c-1)?()=>p(c-1):void 0,onNext:Reflect.has(a.comicMap,c+1)?()=>p(c+1):void 0})};for(const[c,d]of t.querySelectorAll(".xControl > a").entries()){const l=d.parentElement.nextElementSibling;n("comicMap",c,{getImgList:()=>[...l.querySelectorAll("img")].map(f=>f.dataset.src??f.src)}),d.addEventListener("click",()=>l.style.display&&p(c))}s();return}await t.wait(()=>t.querySelectorAll(".entry-content img").length),n("comicMap","",{getImgList:()=>t.querySelectorAll(".entry-content img").map(p=>p.dataset.src||p.src)})})();break}case"2025copy.com":case"www.2025copy.com":case"copy20.com":case"www.copy20.com":case"mangacopy.com":case"www.mangacopy.com":{const t=require("solid-js/web"),o=require("solid-js"),a=require("helper"),n=require("main"),r=new class{headers={webp:"1",region:"1","User-Agent":"COPY/2.0.7|",version:"2.0.7",source:"copyApp",referer:"com.copymanga.app-2.0.7"};get=(c,d,...l)=>n.request(c,{responseType:"json",headers:this.headers,...d},...l)},s=new class{headers={"User-Agent":navigator.userAgent,referer:location.href};get=(c,d,...l)=>n.request(`https://mapi.copy20.com${c}`,{responseType:"json",headers:this.headers,fetch:!1,...d},...l)},_=c=>{let d;const l=new CSSStyleSheet;document.adoptedStyleSheets.push(l);const m=async()=>{d||(async()=>{d=document.createElement("a");const u=await a.wait(()=>a.querySelector(".table-default-right"));d.target="_blank",u.firstElementChild?.before(d);const w=document.createElement("span");w.textContent="\u6700\u5F8C\u95B1\u8B80\uFF1A",u.firstElementChild?.before(w)})(),d.textContent="\u7372\u53D6\u4E2D",d.removeAttribute("href");const h=(await s.get(`/api/v3/comic2/${c}/query?platform=3`)).response?.results?.browse;if(!h){d.textContent=h===null?"\u7121":"\u672A\u8FD4\u56DE\u6578\u64DA";return}const b=h.chapter_id;if(!b){d.textContent="\u63A5\u53E3\u7570\u5E38";return}await l.replace(`ul a[href*="${b}"] {
        color: #fff !important;
        background: #1790E6;
      }`),d.href=`${location.pathname}/chapter/${b}`,d.textContent=h.chapter_name};setTimeout(m),document.addEventListener("visibilitychange",m)},p=async(c,d)=>{const{response:{results:l}}=await r.get(`/comicdetail/${c}/chapters`,{errorText:"\u52A0\u8F09\u6F2B\u756B\u76EE\u9304\u5931\u6557"}),f=await(async(S,x,$)=>{const v=await crypto.subtle.decrypt({name:"AES-CBC",iv:new TextEncoder().encode($)},await crypto.subtle.importKey("raw",new TextEncoder().encode(x),{name:"AES-CBC"},!1,["decrypt"]),new Uint8Array(S.match(/.{1,2}/g).map(y=>Number.parseInt(y,16))).buffer);return JSON.parse(new TextDecoder().decode(v))})(l.slice(16),unsafeWindow.dio||"xxymanga.zzl.key",l.slice(0,16));a.log(f);const{build:{type:h},groups:b}=f,u=S=>{const x=Object.fromEntries(h.map(({id:$})=>[$,[]]));for(const $ of S.chapters)x[$.type].push($);switch(d){case"mobile":for(const $ of a.querySelectorAll(".van-divider"))$.remove();return(()=>{var $=t.template('<div class="detailsTextContentTabs van-tabs van-tabs--line">')();return t.insert($,t.createComponent(o.For,{each:h,children:({id:v,name:y})=>t.createComponent(o.Show,{get when(){return x[v].length},get children(){return[(()=>{var g=t.template('<div class=van-tabs__wrap><div role=tablist class="van-tabs__nav van-tabs__nav--line"><div role=tab class="van-tab van-tab--active"><span class="van-tab__text van-tab__text--ellipsis"><span></span></span></div><div class=van-tabs__line style=transition-duration:0.3s>')(),k=g.firstChild,I=k.firstChild,L=I.firstChild,C=L.firstChild,M=I.nextSibling;return t.setStyleProperty(k,"background","transparent"),t.insert(C,y),t.setStyleProperty(M,"width","0.24rem"),t.setStyleProperty(M,"transform","translateX(187.5px) translateX(-50%)"),g})(),(()=>{var g=t.template('<div class=van-tab__pane><div class="chapterList van-grid"style=padding-left:0.24rem>')(),k=g.firstChild;return t.insert(k,t.createComponent(o.For,{get each(){return x[v]},children:I=>(()=>{var L=t.template('<div class="chapterItem oneLines van-grid-item"style=flex-basis:25%;padding-right:0.24rem;margin-top:0.24rem><a class="van-grid-item__content van-grid-item__content--center"><span class=van-grid-item__text>')(),C=L.firstChild,M=C.firstChild;return t.insert(M,()=>I.name),t.effect(T=>{var P=S.last_chapter.uuid===I.id,N=`/comic/${c}/chapter/${I.id}`;return P!==T.e&&L.classList.toggle("red",T.e=P),N!==T.t&&t.setAttribute(C,"href",T.t=N),T},{e:void 0,t:void 0}),L})()})),g})()]}})})),$})();case"web":return[(()=>{var $=t.template("<span>")();return t.insert($,()=>S.name),$})(),(()=>{var $=t.template('<div class=table-default><div class=table-default-title><ul class="nav nav-tabs"role=tablist></ul><div class=table-default-right><span>\u66F4\u65B0\u5167\u5BB9\uFF1A</span><a target=_blank></a><span>\u66F4\u65B0\u6642\u9593\uFF1A</span><span></span></div></div><div class=table-default-box><div class=tab-content>')(),v=$.firstChild,y=v.firstChild,g=y.nextSibling,k=g.firstChild,I=k.nextSibling,L=I.nextSibling,C=L.nextSibling,M=v.nextSibling,T=M.firstChild;return t.insert(y,t.createComponent(o.For,{each:h,children:({id:P,name:N})=>(()=>{var E=t.template("<li class=nav-item><a class=nav-link data-toggle=tab role=tab aria-selected=false>")(),D=E.firstChild;return t.insert(D,N),t.effect(A=>{var R=x[P].length===0,O=`#${S.path_word}${N}`;return R!==A.e&&D.classList.toggle("disabled",A.e=R),O!==A.t&&t.setAttribute(D,"href",A.t=O),A},{e:void 0,t:void 0}),E})()})),t.insert(I,()=>S.last_chapter.name),t.insert(C,()=>S.last_chapter.datetime_created),t.insert(T,t.createComponent(o.For,{each:h,children:({id:P,name:N})=>(()=>{var E=t.template('<div role=tabpanel class="tab-pane fade"><ul>')(),D=E.firstChild;return t.insert(D,t.createComponent(o.For,{get each(){return x[P]},children:A=>(()=>{var R=t.template("<a target=_blank><li>")(),O=R.firstChild;return t.setStyleProperty(R,"display","block"),t.insert(O,()=>A.name),t.effect(z=>{var q=`/comic/${c}/chapter/${A.id}`,B=A.name;return q!==z.e&&t.setAttribute(R,"href",z.e=q),B!==z.t&&t.setAttribute(R,"title",z.t=B),z},{e:void 0,t:void 0}),R})()})),t.effect(()=>t.setAttribute(E,"id",`${S.path_word}${N}`)),E})()})),t.effect(()=>t.setAttribute(I,"href",`/comic/${c}/chapter/${S.last_chapter.comic_id}`)),$})()];default:return t.createComponent(o.For,{each:h,children:({id:$,name:v})=>t.createComponent(o.Show,{get when(){return x[$].length},get children(){var y=t.template("<div class=card style=max-width:100em><div class=card-body><h2 class=card-title></h2><ul>")(),g=y.firstChild,k=g.firstChild,I=k.nextSibling;return t.setStyleProperty(y,"margin","1em auto"),t.insert(k,v),t.insert(I,t.createComponent(o.For,{get each(){return x[$]},children:L=>(()=>{var C=t.template('<a class="btn btn-outline-primary">')();return t.insert(C,()=>L.name),t.effect(M=>{var T=S.last_chapter.uuid===L.id,P=`/comic/${c}/chapter/${L.id}`;return T!==M.e&&C.classList.toggle("active",M.e=T),P!==M.t&&t.setAttribute(C,"href",M.t=P),M},{e:void 0,t:void 0}),C})()})),y}})})}};let w;switch(d){case"mobile":w=a.querySelector(".detailsTextContent");for(const S of a.querySelectorAll("button.van-dialog__confirm"))S.click();break;case"web":w=a.querySelector(".upLoop");break;default:w=a.querySelector("main"),w.textContent="",a.useStyle("ul .btn { height: fit-content; width: fit-content; margin: 1em; }");break}t.render(()=>t.createComponent(o.For,{get each(){return Object.values(b)},children:u}),w);for(const S of a.querySelectorAll(".upLoop .table-default-title"))S.querySelector(".nav-link:not(.disabled)")?.click()};(async()=>{const c=document.cookie.split("; ").find(m=>m.startsWith("token="))?.replace("token=","");c&&(Reflect.set(r.headers,"Authorization",`Token ${c}`),Reflect.set(s.headers,"Authorization",`Token ${c}`));let d="",l="";if(location.href.includes("/chapter/")?[,,d,,l]=location.pathname.split("/"):location.href.includes("/comicContent/")&&([,,,d,l]=location.pathname.split("/")),d&&l){const{setState:m}=await n.useInit("copymanga"),f=a.querySelector("main .img+.title");f&&(f.textContent="ComicRead \u63D0\u793A\u60A8\uFF1A\u4F60\u8A2A\u554F\u7684\u5167\u5BB9\u66AB\u4E0D\u5B58\u5728\uFF0C\u8ACB\u9EDE\u9078\u53F3\u4E0B\u89D2\u6309\u9215\u5617\u8A66\u52A0\u8F09\u6F2B\u756B"),m("comicMap","",{async getImgList(){f&&(f.textContent="\u6F2B\u756B\u52A0\u8F09\u4E2D\uFF0C\u8ACB\u5750\u548C\u653E\u5BEC");const b=await s.get(`/api/v3/comic/${d}/chapter2/${l}?platform=3`,{noCheckCode:!0});if(b.status!==200){const x=`\u6F2B\u756B\u52A0\u8F09\u5931\u6557\uFF1A${b.response.message||b.status}`;throw f&&(f.textContent=x),new Error(x)}if(f){f.textContent="\u6F2B\u756B\u52A0\u8F09\u6210\u529F\u{1F973}";const{chapter:{name:x},comic:{name:$}}=b.response.results;document.title=`${$} - ${x} - \u62F7\u8C9D\u6F2B\u756B \u62F7\u8D1D\u6F2B\u753B`}if(f??!a.querySelector(".comicContent-next")){const{chapter:{next:x,prev:$}}=b.response.results;m("manga",{onNext:x?()=>location.assign(`/comic/${d}/chapter/${x}`):void 0,onPrev:$?()=>location.assign(`/comic/${d}/chapter/${$}`):void 0})}else m("manga",{onNext:a.querySelectorClick(".comicContent-next a:not(.prev-null)"),onPrev:a.querySelectorClick(".comicContent-prev:not(.index,.list) a:not(.prev-null)")});const u=[],{words:w,contents:S}=b.response.results.chapter;for(let x=0;x<S.length;x++)u[w[x]]=S[x].url.replace(/(?<=(\/|\.))c800x/,"c1500x");return u}}),m("manga","commentList",await(async()=>{const b=location.pathname.split("/").at(-1);return(await s.get(`/api/v3/roasts?chapter_id=${b}&limit=100&offset=0&_update=true`,{errorText:"\u83B7\u53D6\u6F2B\u753B\u8BC4\u8BBA\u5931\u8D25"})).response.results.list.map(({comment:w})=>w)})());return}if(!l&&location.href.includes("/comic/")){if([,d]=location.href.split("/comic/"),!d)return;let m;const f=location.href.includes("/h5/");if(document.title==="404 - \u62F7\u8C9D\u6F2B\u756B"?m=f?"mobile":"404":f?(await a.wait(()=>a.querySelector(".van-toast__text")?.parentElement?.style.display==="none"),m=await a.wait(()=>{if(a.querySelector(".isBan")?.textContent?.includes("\u4E0D\u63D0\u4F9B\u95B1\u89BD"))return"mobile";const h=a.querySelector(".van-dialog__message");if(h?.textContent?.includes("\u6F2B\u756B\u672A\u627E\u5230")){h.textContent=`\u6F2B\u756B\u672A\u627E\u5230!
\u8ACB\u5750\u548C\u653E\u5BEC\uFF0C\u7B49\u5F85\u76EE\u9304\u751F\u6210`;for(const b of a.querySelectorAll(".detailsTextContentTabs"))b.remove();return"mobile"}},1e3)):(a.querySelector(".wargin")?.textContent?.includes("\u4E0D\u63D0\u4F9B\u95B1\u89BD")||!await a.wait(()=>a.querySelector(".upLoop .table-default-title"),1e3))&&(m=a.querySelector(".comicParticulars-title")?"web":"404"),m){const h=a.querySelector(".isBan, .wargin");h&&(h.style.textDecoration="line-through");const b=a.querySelector("main .img+.title");b&&(b.textContent="ComicRead \u63D0\u793A\u60A8\uFF1A\u4F60\u8A2A\u554F\u7684\u5167\u5BB9\u66AB\u4E0D\u5B58\u5728\uFF0C\u8ACB\u5750\u548C\u653E\u5BEC\uFF0C\u7B49\u5F85\u76EE\u9304\u751F\u6210");try{await p(d,m)}catch{b&&(b.textContent="ComicRead \u63D0\u793A\u60A8\uFF1A\u76EE\u9304\u751F\u6210\u5931\u6557\u{1F622}"),n.toast.error("\u76EE\u9304\u751F\u6210\u5931\u6557\u{1F622}",{duration:Number.POSITIVE_INFINITY})}}!f&&c&&_(d)}})();break}case"www.pixiv.net":{let t=[];options={name:"pixiv",getImgList:()=>t,SPA:{async isMangaPage(){const o=Number(location.pathname.split("/")[2]);return!o||!location.pathname.startsWith("/artworks/")?(t.length=0,!1):(t=(await main.request(`/ajax/illust/${o}/pages`,{responseType:"json"})).response.body.map(n=>n.urls.original),t.length>1)}},initOptions:{autoShow:!1,defaultOption:{pageNum:1}}};break}case"manhua.zaimanhua.com":{options={name:"zaiManHua",wait:()=>!!helper.querySelector(".scrollbar-demo-item"),getImgList:()=>unsafeWindow.__NUXT__.data.getChapters?.data?.chapterInfo?.page_url,SPA:{isMangaPage:()=>location.pathname.startsWith("/view/"),getOnNext:()=>helper.querySelectorClick("#next_chapter"),getOnPrev:()=>helper.querySelectorClick("#prev_chapter")}};break}case"m.zaimanhua.com":{const t=async(a,n)=>{const r=await main.request(`https://v4api.zaimanhua.com/app/v1/comic/chapter/${a}/${n}?_v=15`,{responseType:"json"});return r.response.errno&&main.toast.error(`${helper.t("alert.comic_load_error")}: ${r.response.errmsg}`,{throw:!0}),r.response.data.data},o=async a=>{const n=await main.request(`https://v4api.zaimanhua.com/app/v1/comic/detail/${a}?_v=15`,{responseType:"json"});return n.response.errno&&main.toast.error(`${helper.t("alert.comic_load_error")}: ${n.response.errmsg}`,{throw:!0}),n.response.data.data};options={name:"zaiManHua",async getImgList({setState:a}){const n=new URLSearchParams(location.search),r=Number(n.get("comic_id")),s=Number(n.get("chapter_id"));if(!r||!s)throw new Error(helper.t("site.changed_load_failed"));const _=await o(r),p=(_.chapters.length===1?_.chapters[0]:_.chapters.find(l=>l.data.find(m=>m.chapter_id===s))).data;p.sort((l,m)=>l.chapter_order-m.chapter_order);const c=p.findIndex(l=>l.chapter_id===s);return a("manga",{onPrev:c>0?()=>location.assign(`/pages/comic/page?comic_id=${r}&chapter_id=${p[c-1].chapter_id}`):void 0,onNext:c+1<p.length?()=>location.assign(`/pages/comic/page?comic_id=${r}&chapter_id=${p[c+1].chapter_id}`):void 0}),(await t(r,s)).page_url_hd},SPA:{isMangaPage:()=>location.pathname==="/pages/comic/page"}};break}case"terra-historicus.hypergryph.com":{const t=()=>`https://terra-historicus.hypergryph.com/api${location.pathname}`,o=a=>async()=>{const n=await main.request(`${t()}/page?pageNum=${a+1}`);return JSON.parse(n.responseText).data.url};options={name:"terraHistoricus",wait:()=>!!helper.querySelector(".HG_COMIC_READER_main"),async getImgList(){const n=(await main.request(t(),{responseType:"json"})).response.data.pageInfos;if(n.length===0&&location.pathname.includes("episode"))throw new Error("\u83B7\u53D6\u56FE\u7247\u5217\u8868\u65F6\u51FA\u9519");return helper.plimit(helper.range(n.length,o))},SPA:{isMangaPage:()=>location.href.includes("episode"),getOnPrev:()=>helper.querySelectorClick("footer .HG_COMIC_READER_prev a"),getOnNext:()=>helper.querySelectorClick("footer .HG_COMIC_READER_prev+.HG_COMIC_READER_buttonEp a")}};break}case"18comic.ink":case"jmcomic-zzz.one":case"jmcomic-zzz.org":case"jm18c-twie.vip":case"jm18c-tin.org":case"jm18c-qwq.cc":case"18comic.org":case"18comic.vip":{const t=require("helper"),o=require("main");(async()=>{if(!location.pathname.includes("/photo/"))return;const{setState:a}=await o.useInit("jm");for(;!unsafeWindow?.onImageLoaded;){if(document.readyState==="complete"){o.toast.error("\u65E0\u6CD5\u83B7\u53D6\u56FE\u7247",{duration:Number.POSITIVE_INFINITY});return}await t.sleep(100)}a("manga",{onPrev:t.querySelectorClick(()=>t.querySelector(".menu-bolock-ul .fa-angle-double-left")?.parentElement),onNext:t.querySelectorClick(()=>t.querySelector(".menu-bolock-ul .fa-angle-double-right")?.parentElement)});const n=t.querySelectorAll(".scramble-page:not(.thewayhome) > img");if(unsafeWindow.aid<unsafeWindow.scramble_id||unsafeWindow.speed==="1")return a("comicMap","",{getImgList:()=>n.map(p=>p.dataset.original??"")});const r=async p=>{try{return await o.request(p,{responseType:"blob",fetch:!0,noTip:!0},3)}catch{return await o.request(p,{responseType:"blob",revalidate:!0,fetch:!1},3)}},s=async p=>{if(p.src.startsWith("blob:"))return p.src;const c=p.dataset.original,d=await r(p.dataset.original);if(d.response.size===0)return o.toast.warn(`\u4E0B\u8F7D\u539F\u56FE\u65F6\u51FA\u9519: ${p.dataset.page}`),"";p.src=`${URL.createObjectURL(d.response)}#${p.src}`;try{await t.waitImgLoad(p,1e3*10)}catch{return URL.revokeObjectURL(p.src),p.src=c,o.toast.warn(`\u52A0\u8F7D\u539F\u56FE\u65F6\u51FA\u9519: ${p.dataset.page}`),""}try{p.nextElementSibling?.tagName==="CANVAS"&&p.nextElementSibling.remove(),unsafeWindow.onImageLoaded(p);const l=await t.canvasToBlob(p.nextElementSibling,"image/webp",1);if(URL.revokeObjectURL(p.src),!l)throw new Error("\u8F6C\u6362\u56FE\u7247\u65F6\u51FA\u9519");return`${URL.createObjectURL(l)}#.webp`}catch(l){return p.src=c,o.toast.warn(`\u8F6C\u6362\u56FE\u7247\u65F6\u51FA\u9519: ${p.dataset.page}, ${l.message}`),""}};await t.wait(()=>{const p=t.querySelectorAll(".lazy-loaded").length;return p>0&&t.querySelectorAll("canvas").length-p<=1});const _=p=>t.plimit(n.map((c,d)=>async()=>{p(d,await s(c))}));a("comicMap","",{getImgList:({dynamicLoad:p})=>p(_,n.length)})})().catch(a=>t.log.error(a));break}case"tw.manhuagui.com":case"m.manhuagui.com":case"www.mhgui.com":case"www.manhuagui.com":{if(!/\/comic\/\d+\/\d+\.html/.test(location.pathname))break;let comicInfo;try{const dataScript=helper.querySelectorAll("body > script:not([src])").find(t=>t.innerHTML.startsWith("window["));if(!dataScript)throw new Error(helper.t("site.changed_load_failed"));comicInfo=JSON.parse(eval(dataScript.innerHTML.slice(26)).match(/(?<=\()\{.+\}/)[0])}catch{main.toast.error(helper.t("site.changed_load_failed"));break}helper.useStyle("#smh-msg-box { z-index: 2147483647 !important }");const handlePrevNext=t=>{if(t===0)return;const o=location.pathname.replace(/(?<=\/)\d+(?=\.html)/,`${t}`);return()=>location.assign(o)};options={name:"manhuagui",getImgList(){const t=Object.entries(comicInfo.sl).map(o=>`${o[0]}=${o[1]}`).join("&");if(comicInfo.files)return comicInfo.files.map(o=>`${unsafeWindow.pVars.manga.filePath}${o}?${t}`);if(comicInfo.images){const{origin:o}=new URL(helper.querySelector("#manga img").src);return comicInfo.images.map(a=>`${o}${a}?${t}`)}return main.toast.error(helper.t("site.changed_load_failed"),{throw:!0}),[]},onNext:handlePrevNext(comicInfo.nextId),onPrev:handlePrevNext(comicInfo.prevId)};break}case"www.manhuaren.com":case"m.1kkk.com":case"www.1kkk.com":case"tel.dm5.com":case"en.dm5.com":case"cnc.dm5.com":case"www.dm5.cn":case"www.dm5.com":{if(!Reflect.has(unsafeWindow,"DM5_CID"))break;const imgNum=unsafeWindow.DM5_IMAGE_COUNT??unsafeWindow.imgsLen;if(!(Number.isSafeInteger(imgNum)&&imgNum>0)){main.toast.error(helper.t("site.changed_load_failed"));break}const getPageImg=async i=>{const res=await unsafeWindow.$.ajax({type:"GET",url:"chapterfun.ashx",data:{cid:unsafeWindow.DM5_CID,page:i,key:unsafeWindow.$("#dm5_key").length>0?unsafeWindow.$("#dm5_key").val():"",language:1,gtk:6,_cid:unsafeWindow.DM5_CID,_mid:unsafeWindow.DM5_MID,_dt:unsafeWindow.DM5_VIEWSIGN_DT,_sign:unsafeWindow.DM5_VIEWSIGN}});return eval(res)},handlePrevNext=(t,o)=>helper.querySelectorClick(()=>helper.querySelector(t)??helper.querySelectorAll(".view-bottom-bar a").find(a=>a.textContent?.includes(o)));options={name:"dm5",getImgList({dynamicLoad:t}){return Array.isArray(unsafeWindow.newImgs)&&unsafeWindow.newImgs.every(helper.isUrl)?unsafeWindow.newImgs:t(async o=>{const a=new Set;for(;a.size<imgNum;)for(const n of await getPageImg(a.size+1))a.has(n)||(a.add(n),o(a.size-1,n))},imgNum)},onPrev:handlePrevNext(".logo_1","\u4E0A\u4E00\u7AE0"),onNext:handlePrevNext(".logo_2","\u4E0B\u4E00\u7AE0"),onExit:t=>t&&helper.scrollIntoView(".postlist")};break}case"www.wnacg03.cc":case"www.wnacg02.cc":case"www.wnacg01.cc":case"www.wn03.ru":case"www.wnacg.com":case"wnacg.com":{const t=helper.querySelector("#bodywrap a.btn");if(t&&(t.style.setProperty("background-color","#607d8b"),t.style.setProperty("background-image","none")),!Reflect.has(unsafeWindow,"imglist"))break;options={name:"wnacg",getImgList:()=>unsafeWindow.imglist.filter(({caption:o})=>o!=="\u559C\u6B61\u7D33\u58EB\u6F2B\u756B\u7684\u540C\u5B78\u8ACB\u52A0\u5165\u6536\u85CF\u54E6\uFF01").map(({url:o})=>new URL(o,location.origin).href)};break}case"www.mangabz.com":case"mangabz.com":{if(!Reflect.has(unsafeWindow,"MANGABZ_CID"))break;const imgNum=unsafeWindow.MANGABZ_IMAGE_COUNT??unsafeWindow.imgsLen;if(!(Number.isSafeInteger(imgNum)&&imgNum>0)){main.toast.error(helper.t("site.changed_load_failed"));break}const getPageImg=async i=>{const res=await unsafeWindow.$.ajax({type:"GET",url:"chapterimage.ashx",data:{cid:unsafeWindow.MANGABZ_CID,page:i,key:"",_cid:unsafeWindow.MANGABZ_CID,_mid:unsafeWindow.MANGABZ_MID,_dt:unsafeWindow.MANGABZ_VIEWSIGN_DT,_sign:unsafeWindow.MANGABZ_VIEWSIGN}});return eval(res)},handlePrevNext=(t,o)=>helper.querySelectorClick(()=>helper.querySelector(t)??helper.querySelectorAll(".bottom-bar-tool a").find(a=>a.textContent?.includes(o)));options={name:"mangabz",getImgList:({dynamicLoad:t})=>t(async o=>{const a=new Set;for(;a.size<imgNum;)for(const n of await getPageImg(a.size+1))a.has(n)||(a.add(n),o(a.size-1,n))},imgNum),onNext:handlePrevNext('body > .container a[href^="/"]:last-child',"\u4E0B\u4E00"),onPrev:handlePrevNext('body > .container a[href^="/"]:first-child',"\u4E0A\u4E00")};break}case"komiic.com":{const t=`
